...
Nichts, was Sie tun, sollte den alten Code eines Kunden zerstören. Consider only `/libs` to contain product code that can be updated during an upgrade. The `/apps` section of the repository is project code, and the `/etc` section contains custom configurations that need to be preserved. Generally, do not overwrite anything in `/apps`, `/content` and `/home`. Nach einer Aktualisierung sollten alter Projektcode, Konfigurationen und Inhalt weiterhin wie vor der Aktualisierung funktionieren.
