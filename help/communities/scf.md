...
* Für Look, Feel und UX
   * JS-Vorlage, CSS und [Javascript erweitern/überschreiben](client-customize.md#extending-javascript) ändern
...
1. Übergibt die Ressource (resourceType) an `SocialComponentFactoryManager`und erhält eine SocialComponentFactory, die eine `SocialComponent`für die Ressource darstellt.

1. Ruft die Factory auf und erhält einen `SocialComponent`Status, der die Ressource und Anfrage verarbeiten kann.
...
Informationen zur Handhabung von benutzergenerierten Inhalten, die im [Community-Inhaltsspeicher](working-with-srp.md) gespeichert sind, finden Sie unter
