---
cloud: experience-cloud
product: adobe experience manager
audience: end-user
user-guide-title: AEM 6.4 Communities User Guide
translation-type: tm+mt
source-git-commit: 1e867debcc1bfeed3abc4240796c9f2c2850167f

---


# AEM 6.4 Communities User Guide {#communities}

+ [Communities-Benutzerhandbuch](home.md)
+ Introduction to AEM Communities {#introduction}
   + [Communities Sites](administer-landing.md)
   + [Übersicht über AEM Communities](overview.md)
   + [Neue Funktionen in AEM 6.4 Communities](whats-new-aem-communities.md)
   + [Versionshinweise zu AEM Communities](/help/release-notes/communities-release-notes.md)
   + [Einstieg in AEM Communities](getting-started.md)
   + [Erste Schritte mit AEM Communities zur Aktivierung](getting-started-enablement.md)
   + [Ersteinrichtung](setup.md)
   + [Erstellen einer neuen Community-Site](create-site.md)
   + [Die veröffentlichte Site](published-site.md)
   + [Erstellen von verschachtelten Gruppen](nested-groups.md)
   + [Ersteinrichtung für Aktivierung](enablement-setup.md)
   + [Erstellen einer neuen Community-Site für die Aktivierung](enablement-create-site.md)
   + [Einrichten und Zuweisen von Aktivierungsressourcen](resource.md)
   + [Die veröffentlichte Website zur Aktivierung](enablement-published-site.md)
+ Bereitstellung {#deploy}
   + [Bereitstellen von Communities](deploy-communities.md)
   + [Upgrade auf AEM 6.4 Communities](upgrade.md)
   + [Empfohlene Topologien für Communities](topologies.md)
   + [Konfigurieren von Dispatcher für Communities](dispatcher.md)
+ Verwalten {#administer}
   + [Verwaltungsdienst für Benutzer und benutzergenerierte Inhalte in AEM Communities](user-ugc-management-service.md)
   + [Communities Sites](administer-landing.md)
   + [Communities Konsolen](consoles.md)
   + [Analytics-Konfiguration für Communities-Funktionen](analytics.md)
   + [Konfiguration von Aktivierungsfunktionen](enablement.md)
   + [Messaging konfigurieren](messaging.md)
   + [Moderieren von Community-Inhalten](moderate-ugc.md)
   + [Konfigurieren der E-Mail](email.md)
   + [Communities-Benachrichtigungen](notifications.md)
   + [Communities-Abonnements](subscriptions.md)
   + [Communities - Bewertung und Abzeichen](implementing-scoring.md)
   + [Verwalten von Benutzern und Benutzergruppen](users.md)
   + [Community Groups-Konsole](groups.md)
   + [Social-Anmeldung bei Facebook und Twitter](social-login.md)
   + [SRP - Gemeinschaftlicher Inhaltsspeicher](working-with-srp.md)
   + [Communities-Sites-Konsole](sites-console.md)
   + [Mitglieder und Gruppenverwaltungskonsolen](members.md)
   + [Moderationskonsole](moderation.md)
   + [Communities-Tools](tools.md)
   + [Site-Vorlagen](sites.md)
   + [Gruppenvorlagen](tools-groups.md)
   + [Community-Funktionen](functions.md)
   + [Speicherkonfiguration](srp-config.md)
   + [Badges-Konsole](badges.md)
   + [Aktivierungsressourcen-Konsole](resources.md)
   + [Berichte-Konsole](reports.md)
   + [MySQL-Konfiguration für Aktivierungsfunktionen](mysql.md)
   + [FFmpeg für Communities](ffmpeg.md)
   + [Kontextbezogene Moderation](in-context.md)
   + [Erweiterte Bewertung und Abzeichen](advanced.md)
   + [ASRP - Adobe Storage Resource Provider](asrp.md)
   + [MSRP - MongoDB Storage Resource Provider](msrp.md)
   + [Setup von MongoDB für Demo](demo-mongo.md)
   + [DSRP - Ressourcenanbieter für den relationalen Datenbankspeicher](dsrp.md)
   + [MySQL-Konfiguration für DSRP](dsrp-mysql.md)
   + [JSRP - JCR Storage Resource Provider](jsrp.md)
   + [Solr-Konfiguration für SRP](solr.md)
   + [Tagging von Aktivierungsressourcen](tag-resources.md)
   + [Tagging benutzergenerierter Inhalte](tag-ugc.md)
   + [Übersetzen benutzergenerierter Inhalte](translate-ugc.md)
   + [Communities-Benutzersynchronisierung](sync.md)
   + [Beitragsbeschränkungen für Mitglieder](limits.md)
+ Entwickeln {#develop}
   + [Entwicklungsländer](communities.md)
   + [Social Component Framework](scf.md)
   + [Handbuch der Community-Komponenten](components-guide.md)
   + [Komponenten, Funktionen und Funktionen](essentials.md)
   + [Tutorials](tutorials.md)
   + [Fehlerbehebung](troubleshooting.md)
   + [Kodierungsrichtlinien ](code-guide.md)
   + [Serverseitige Anpassung](server-customize.md)
   + [Clientseitige Anpassung](client-customize.md)
   + [SCF Handlebars Helfers](handlebars-helpers.md)
   + [Übersicht über den Speicher-Ressourcen-Provider](srp.md)
   + [Community-Site-Grundlagen](sites-for-developers.md)
   + [Aktivitäts-Stream-Grundlagen](essentials-activities.md)
   + [Grundlagen der Zuweisung](essentials-assignments.md)
   + [Blog Essentials](blog-developer-basics.md)
   + [Essentials des Kalenders](calendar-basics-for-developers.md)
   + [Kataloggrundlagen](catalog-developer-essentials.md)
   + [Clientlibs für Communities-Komponenten](clientlibs.md)
   + [Kommentare Essentials](essentials-comments.md)
   + [OSGi-Ereignisse für Communities-Komponenten](events.md)
   + [Essentials zu speziellen Inhalten](essentials-featured.md)
   + [Essentials zur Dateibibliothek](essentials-file-library.md)
   + [Forum Essentials](essentials-forum.md)
   + [Community-Gruppengrundlagen](essentials-groups.md)
   + [Grundlegende Informationen](ideation.md)
   + [Leitfaden - Grundlagen](leaderboard.md)
   + [Gefällt mir-Essentials](essentials-liking.md)
   + [Messaging Essentials](essentials-messaging.md)
   + [QnA-Grundlagen](qna-essentials.md)
   + [Bewertungsgrundlagen](rating-basics.md)
   + [Reviews Essentials](reviews-basics.md)
   + [Grundlagen des Rich-Text-Editors](rte.md)
   + [Grundlagen zu Bewertung und Abzeichen](configure-scoring.md)
   + [Grundlagen](search-implementation.md)
   + [Komponenten-Sideloading](sideloading.md)
   + [Grundlagen zu Social-Diagrammen](essentials-socialgraph.md)
   + [SRP und UGC Essentials](srp-and-ugc.md)
   + [Tag Essentials](tag.md)
   + [Tally Essentials](tally.md)
   + [Essentials zur Abstimmung](essentials-voting.md)
   + [Website-Struktur einrichten](setup-website.md)
   + [Erstellen einer SCF-Sandbox](an-scf-sandbox.md)
   + [Anfänglicher Sandbox-Antrag](initial-app.md)
   + [Anfänglicher Sandbox-Inhalt](initial-content.md)
   + [Sandbox-Anwendung entwickeln](develop-app.md)
   + [Entwicklung von Sandbox-Inhalten](develop-content.md)
   + [Clientlibs hinzufügen](add-clientlibs.md)
   + [Komponente &quot;Überlagerungskommentare&quot;](overlay-comments.md)
   + [Kommentarseite erstellen](overlay-create-comments-page.md)
   + [Nodes erstellen](overlay-create-nodes.md)
   + [Erscheinungsbild ändern](overlay-alter-appearance.md)
   + [Komponenten erstellen](extend-create-components.md)
   + [Kommentar zur Beispielseite hinzufügen](extend-sample-page.md)
   + [Komponente &quot;Kommentare erweitern&quot;](extend-comments.md)
   + [Erscheinungsbild ändern (HBS)](extend-alter-appearance.md)
   + [Beispielseite erstellen](create-sample-page.md)
   + [Zugriff auf UGC mit SRP](accessing-ugc-with-srp.md)
   + [Verwenden von Maven für Communities](maven.md)
   + [Benennungskonventionen](naming-conventions.md)
   + [SocialUtils-Umgestaltung](socialutils.md)
+ Authoring {#authoring}
   + [Communitys-Komponenten](author-communities.md)
   + [Grundlagen zu Communities-Komponenten](basics.md)
   + [Aktivitäts-Streams-Funktion](activities.md)
   + [Zuweisungsfunktion](assignments.md)
   + [Blogfunktion](blog-feature.md)
   + [Kalenderfunktion](calendar.md)
   + [Katalogfunktion](catalog.md)
   + [Verwenden von Kommentaren](comments.md)
   + [Funktionen für spezielle Inhalte](featured.md)
   + [Dateibibliothek](file-library.md)
   + [Funktion „Forum“](forum.md)
   + [Community-Gruppen](creating-groups.md)
   + [Ideenfunktion](ideation-feature.md)
   + [Leader-Funktion](enabling-leaderboard.md)
   + [Verwenden von &quot;Gefällt mir&quot;](liking.md)
   + [Messaging-Funktion](configure-messaging.md)
   + [Funktion „Fragen-und-Antworten-Forum“](working-with-qna.md)
   + [Verwenden von Bewertungen](rating.md)
   + [Verwenden von Bewertungen und Bewertungszusammenfassung (Anzeige)](reviews.md)
   + [Suchfunktion](search.md)
   + [Verwenden von Sozialdiagrammen](socialgraph.md)
   + [Aktivitätstrends](trends.md)
   + [Verwenden von einer Social-Tag-Cloud](tagcloud.md)
   + [Verwenden einer Abstimmung](voting.md)
   + [Beispielseite erstellen](create-sample-page.md)