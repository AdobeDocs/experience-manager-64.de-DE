...
description: Mit der Anmeldung über soziale Netzwerke können sich Site-Besucher mit ihrem Facebook- oder Twitter-Konto anmelden.
seo-description: Mit der Anmeldung über soziale Netzwerke können sich Site-Besucher mit ihrem Facebook- oder Twitter-Konto anmelden.
...
Bei der Anmeldung in sozialen Netzwerken handelt es sich um die Möglichkeit, einem Site-Besucher die Möglichkeit zu geben, sich mit seinem Facebook- oder Twitter-Konto anzumelden. Daher sollten Sie zulässige Facebook- oder Twitter-Daten in ihr AEM-Mitgliedsprofil aufnehmen.
...
* *Neue Facebook-App hinzufügen:*
   ...
   * Für die *Kategorie* wird empfohlen, &quot; *Apps für Seiten&quot;zu wählen,* kann jedoch alles sein.
...
1. Konfiguration des Facebook-Cloud-Dienstes erstellen/bearbeiten.
   ...
   * **[!UICONTROL Scope-E-Mail]**: E-Mail-ID des Benutzers sollte von Facebook abgerufen werden.
   * **[!UICONTROL Zu Benutzergruppen]** hinzufügen wählen Sie Benutzergruppe hinzufügen, um eine oder mehrere [Mitgliedsgruppen](https://helpx.adobe.com/experience-manager/6-3/communities/using/users.html) für die Community-Site auszuwählen, der Benutzer hinzugefügt werden sollen.
   >[!NOTE]
   >
...
Das Ergebnis ist eine [Adobe Granite OAuth Application and Provider](https://helpx.adobe.com/experience-manager/6-3/communities/using/social-login.html#adobe-granite-oauth-application-and-provider) -Instanz, die keine weitere Änderung erfordert, es sei denn, es werden zusätzliche Bereiche (Berechtigungen) hinzugefügt. Der Standardbereich ist die Standardberechtigung für die Anmeldung bei Facebook. Wenn Sie zusätzlichen Umfang wünschen, müssen Sie die OSGI-Konfiguration direkt bearbeiten. Wenn Änderungen direkt über das System/die Konsole vorgenommen werden, sollten Sie die Konfiguration des Cloud-Dienstes nicht über die Touch-Benutzeroberfläche bearbeiten, um ein Überschreiben zu vermeiden.
...
* Zusätzliche Felder [innerhalb des Bereichs hinzufügen](#adobe-granite-oauth-application-and-provider)
...
1. Wählen Sie das Stiftsymbol aus, das zur Bearbeitung geöffnet werden soll.
   ...
   * **[!UICONTROL Benutzerpfad]**
      ...
      Speicherort im Repository, an dem Benutzerdaten gespeichert werden. Für eine Community-Site sollte der Pfad der Standard-/Home-/Benutzer-/Community-Pfad sein, um sicherzustellen, dass Mitglieder das Profil einer anderen Person anzeigen können **.
   ...
   * **[!UICONTROL Felder]**
      ...
      Wenn Felder aktiviert sind, werden beim Aufruf der Facebook-Grafik-API die folgenden Felder einbezogen. Die Felder müssen innerhalb des in der Cloud-Dienstkonfiguration definierten Bereichs zulässig sein. Zusätzliche Felder müssen möglicherweise von Facebook genehmigt werden. Verweisen Sie in der Facebook-Dokumentation auf den Abschnitt &quot;Facebook-Anmeldeberechtigungen&quot;. Als Parameter hinzugefügte Standardfelder sind:
   ...
   * **[!UICONTROL Benutzer]** aktualisieren Wenn aktiviert, werden die Benutzerdaten im Repository bei jeder Anmeldung aktualisiert, um Profiländerungen oder zusätzliche angeforderte Daten widerzuspiegeln. Die Option &quot;Standard&quot;ist deaktiviert.
...
1. Geben Sie für die *Callback-URL* https://&lt;server>/ ein.
...
1. Wählen Sie in der globalen Navigation **[!UICONTROL Extras > Cloud-Services > Twitter Social-Anmeldekonfiguration]**.
...
1. Konfiguration des Twitter-Cloud-Dienstes erstellen/bearbeiten.
   ...
   * **[!UICONTROL Titel]** (*erforderlich*) Geben Sie einen Anzeigentitel ein, der die Twitter-App identifiziert. Es wird empfohlen, denselben Namen wie den *Anzeigenamen* für die Twitter-App zu verwenden.

   * **[!UICONTROL Consumer Key]** (*Erforderlich*) Geben Sie den **Consumer (API)-Schlüssel** für die Twitter-App ein. Dadurch wird die [Adobe Granite OAuth Application and Provider](https://helpx.adobe.com/experience-manager/6-3/communities/using/social-login.html#AdobeGraniteOAuthApplicationandProvider) -Instanz identifiziert, die über das Dialogfeld erstellt wurde.

   * **[!UICONTROL Privatanwendergeheimnis]** (*erforderlich*) Geben Sie den geheimen ***Verbraucher(API)-Schlüssel*** für die Twitter-App ein.

   * **[!UICONTROL Benutzer]** erstellen Wenn diese Option aktiviert ist, wird bei der Anmeldung bei einem Twitter-Konto ein AEM-Benutzereintrag erstellt und als Mitglied der ausgewählten Benutzergruppe(n) hinzugefügt. Die Standardeinstellung ist aktiviert (wird dringend empfohlen).
...
* Stellen Sie sicher, dass [Adobe Granite OAuth Authentication Handler](#adobe-granite-oauth-authentication-handler) in allen Instanzen im Veröffentlichungsmodus aktiviert wurde
...
* Starten der veröffentlichten Site in einem BrowserBeispiel: http://localhost:4503/content/sites/engage/en.html
* Anmelden **[!UICONTROL auswählen]**
...
* Profil **[!UICONTROL auswählen]**: Auf der Seite &quot;Profil&quot;werden das Avatarbild, der Vorname und der Nachname des Benutzers angezeigt. Außerdem werden die Informationen aus dem Facebook- oder Twitter-Profil entsprechend den zulässigen Feldern/Parametern angezeigt.
...
1. Suchen Sie Adobe Granite OAuth Application and Provider.
   ...
   * **[!UICONTROL Client-ID]**
      ...
      (*Erforderlich*) Die beim Erstellen des Cloud-Dienstes angegebene Anwendungs-ID.

   * **[!UICONTROL Client-Geheimnis]**

      (*Erforderlich*) Das Anwendungsgeheimnis, das beim Erstellen des Cloud-Dienstes angegeben wurde.

   * **[!UICONTROL Anwendungsbereich]**

      (*Optional*) Der Anbieter kann weitere Möglichkeiten für das Erlaubte anfordern. Der Standardbereich umfasst die Berechtigungen, die für die Bereitstellung von Social-Authentifizierung und Profildaten erforderlich sind.
   ...
   * **[!UICONTROL Gruppen]**
      ...
      (*Empfohlen*) Eine oder mehrere Mitgliedsgruppen, denen erstellte Benutzer hinzugefügt werden. Für AEM Communities wird empfohlen, die Mitgliedsgruppe für die Community-Site aufzulisten.
...
Bei Community-Sites, auf denen sich Hunderttausende von Benutzern mit ihrer Facebook- oder Twitter-Anmeldung registrieren, kann die übergreifende Leistung der Abfrage, die ausgeführt wird, wenn ein Site-Besucher seine Social-Anmeldung verwendet, durch Hinzufügen des folgenden Oak-Index verbessert werden.
...
1. Unter Knoten /oak:index/ntBaseLucene-oauth/indexRules/nt:base/properties:
   ...
   * cqTags in oauthid-123xxxx umbenennen
...
**** &amp;ast; Ersetzen Sie für den **Namen** oauthid-*123* den Wert *123* durch die Facebook- ***App-ID*** oder den Twitter-API-Schlüssel ********** [](social-login.md#adobe-granite-oauth-application-and-provider)(Consumer), der den Wert der in der Konfiguration vonClient-IDsvon Adobe Granite OAuth Application undProviderconfiguration festgelegten Wert darstellt.
