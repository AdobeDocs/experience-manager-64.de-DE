...
Die wichtigsten Funktionen und Verbesserungen, die in [!DNL Adobe Experience Manager] 6.4.8.4 sind:
...
* Benutzer können keine Assets einchecken, nachdem sie diese in bearbeitet haben [!DNL Adobe InDesign] und erhalten einen Fehler wegen fehlender Berechtigungen (NPR-35340).
...
Informationen zu CFP und anderen Veröffentlichungstypen finden Sie unter [AEM Update der Versionsdefinitionen](https://docs.adobe.com/content/help/en/experience-manager-65/deploying/deploying/update-release-vehicle-definitions.html)
...
* Im [!UICONTROL Viewer-Vorgaben-Editor]bei der Bearbeitung [!UICONTROL PanoramicImage/PanoramicImage_VR] voreingestellt, in der `PanoramicView` -Komponente, die `PANORAMICVIEW_AUTOROTATE` -Modifikatorbeschriftung ist nicht verfügbar (CQ-4302043).
...
* Wenn die Variable `RolloutConfigManagerFactoryImpl` keine Rollout-Konfiguration laden kann, versucht sie nicht, die fehlenden Konfigurationen zu laden. Es werden die zwischengespeicherten Konfigurationen zurückgegeben (NPR-34091).
* Nach Verwendung der Bearbeitungsoption &quot;Quell-HTML-bearbeiten&quot;in der Text-Kernkomponente die Klasse aus `em` -Tag entfernt wird (NPR-34080).
...
* Die `cq:acLinks` und `cq:acUUID` Eigenschaften für [!DNL Adobe Campaign] auf der Kopie werden beim Kopieren und Einfügen entfernt (NPR-33793).
...
* Im Inline-Bearbeitungsmodus [!DNL RTE], funktioniert das Ziehen eines Bildes nicht für die Textkomponente (NPR-33579).
* Es ist möglich, eine Komponente in einer Blueprint-Seite mit demselben Namen wie die Seite zu erstellen. Während des Rollouts wird eine solche Komponente durch Nachfüllen umbenannt `_msm_moved`. Die Komponente wird jedoch an das Ende der [!UICONTROL Absatzsystem] (NPR-33534).
...
* Weiterleiten an [!DNL Experience Manager] Seite mit Anker funktioniert nicht in der -Autoreninstanz, da `PageRedirectServlets` fügt Abfragezeichenfolgen nach einem URL-Fragment oder Anker ein (NPR-34287).
* `PageRedirectServlet` appends `.html` nach der Sling-Zuordnung, die zu Link-Fehlern führte (NPR-34271).
* Sie können die [!DNL Live Copy] einer Seite und Vererbung beschädigt werden, wie im Editor-Modus angezeigt. In den Seiteneigenschaften zeigt das Symbol für Vererbung jedoch fälschlicherweise an, dass die Vererbung vorhanden und nicht beschädigt ist (NPR-34096).
...
* Die Standardwerte für die **[!UICONTROL Verbindungs-Timeout]** und **[!UICONTROL Socket-Timeout]** Die Einstellungen für die Konfiguration des Standardagenten (Veröffentlichungsinstanz) sind nicht angegeben (NPR-33708).
...
* Die Inhaltsbeschreibung für ein adaptives Formularfeld zeigt ein Absatz-HTML-Tag an (NPR-34117).
...
* Bei der Installation [!DNL Experience Manager] 6.4.8.1, die [!UICONTROL Aufgaben] Die Liste der Elemente wird nicht als Links angezeigt. Der Text für [!UICONTROL Aufgaben] -Elemente enthalten HTML-Tags (NPR-34318).
...
* Der anonyme Zugriff auf die CRXDE Lite ist zur Erhöhung der Sicherheit nicht zulässig. Stattdessen werden die Benutzer zum Anmeldebildschirm weitergeleitet. Siehe [Entwicklung mit CRXDE Lite](/help/sites-developing/developing-with-crxde-lite.md).
...
* Dispatcher-Konsistenzprüfung wird angezeigt `Invalid cookie header` Warnmeldung in den Protokolldateien (NPR-33630).
...
* Die [!DNL Sling] Filter wird nicht aufgerufen, wenn die `sling:match` Zuordnungseintrag wird unter erstellt `/etc/maps` (NPR-33308).
...
* A `NullPointerException` in den Protokollen wird beim Ausführen eines Übersetzungsauftrags ein Fehler angezeigt (NPR-32220).
...
* Autoren werden nach der Erstellung einer neuen Gruppe nicht an die [!UICONTROL Community-Gruppe] Abschnitt [!DNL Internet Explorer] 11 (NPR-33202).
* Beim Zugriff auf die [!UICONTROL Aktivitäts-Stream] Seite (NPR-33152).
* Bearbeiten von [!DNL Communities] das Miniaturbild der Gruppe zu ändern, wird das Miniaturbild der Gruppe nicht aktualisiert (NPR-32603).
...
* Die [!UICONTROL Timeline] in der linken Leiste dauert das Laden länger als erwartet (NPR-32850).
...
1. Laden Sie das entsprechende Formular-Add-On-Paket herunter, das unter [AEM Forms-Versionen](https://experienceleague.adobe.com/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases.html#forms-updates) für Ihr Betriebssystem.
1. Installieren Sie das Formular-Add-On-Paket wie unter [Installieren von Add-On-Paketen für AEM Formulare](https://docs.adobe.com/content/help/en/experience-manager-64/forms/install-aem-forms/osgi-installation/installing-configuring-aem-forms-osgi.html#install-aem-forms-add-on-package).
...
>[!NOTE]
>
>UberJar und andere zugehörige Artefakte sind im Maven Central Repository anstelle des Adobe Public Maven-Repositorys (repo.adobe.com) verfügbar. Die UberJar-Hauptdatei wird in `uber-jar-<version>.jar`. Daher gibt es keine `classifier`, mit `apis` als Wert für die `dependency` -Tag.
