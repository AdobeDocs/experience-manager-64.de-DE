...
1. Stellen Sie sicher, dass alle erforderlichen Bibliotheken extrahiert werden, indem Sie den Ordner mit dem Befehl überprüfen, `ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/`wobei der Ordnername mit der Bündel-ID erstellt wird. Der Befehl lautet beispielsweise, `ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle588/data/binaries/` wenn die Bündel-ID `588`lautet.
...
1. Fügen Sie der conf-Datei `/aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/` mithilfe des `cat SWitchEngineLibs.conf` Befehls den Pfad hinzu.
