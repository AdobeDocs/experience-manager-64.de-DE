...
1. Stellen Sie sicher, dass alle erforderlichen Bibliotheken extrahiert werden, indem Sie den Ordner mit dem Befehl `ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/`, wobei der Ordnername mithilfe der Bundle-ID erstellt wird. Beispielsweise lautet der Befehl `ls -la /aem64/author/crx-quickstart/launchpad/felix/bundle588/data/binaries/` if bundle id is `588`.
...
1. Hinzufügen `/aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/` Pfad zur conf-Datei mit `cat SWitchEngineLibs.conf` Befehl.

1. Ausführen `ldconfig` -Befehl, um die erforderlichen Links und den Cache zu erstellen.

1. Bearbeiten Sie in dem Konto, das zum Starten von AEM verwendet wird `.bash_profile` -Datei. Hinzufügen `LD_LIBRARY_PATH` indem Sie Folgendes hinzufügen.
