...
description: Best Practices zur Integration einer AEM-Bereitstellung in Adobe Creative Cloud, um Asset-Transfer-Arbeitsabläufe zu optimieren und eine maximale Effizienz zu erzielen
contentOwner: AG
...
|---|---|---|---|
| Freigeben einer kleineren Anzahl (1) von DAM-Assets für einen Creative-Benutzer | ✔✔ | ✔ |  |
| Freigeben einer größeren Anzahl (2) von DAM-Assets für einen Creative-Benutzer | ✔✔ | ✘ | [Brand Portal](https://docs.adobe.com/content/help/en/experience-manager-brand-portal/using/home.html) <br> [Asset-Freigabe](assets-finder-editor.md) |
| Freigabe von DAM-Assets für Benutzer, die Zugriff auf DAM haben | ✔✔ | ✔ | [Linkfreigabe](link-sharing.md) |
| Freigabe von DAM-Assets für Benutzer, die keinen Zugriff auf DAM haben | ✘ | ✔✔ | [Brand Portal](https://docs.adobe.com/content/help/en/experience-manager-brand-portal/using/home.html) <br> [Asset-Freigabe](assets-finder-editor.md) |
| Speichern einer kleineren Anzahl/Menge von Assets in DAM | ✔✔ | ✔ | [Hochladen über die Web-Benutzeroberfläche](managing-assets-touch-ui.md) |
| Speichern Sie mehr Assets in DAM (3) | ✔✔ | ✘ | [Hochladen über die Web-Benutzeroberfläche](managing-assets-touch-ui.md) <br> Custom script / tool |
...
* (1) Kleinere Anzahl von Assets: zum Beispiel ein kleiner Satz von Assets, die mit einem Projekt oder einer Kampagne verbunden sind
* (2) Größere Anzahl von Vermögenswerten: zum Beispiel alle genehmigten Assets in der Organisation
* (3) Verwenden Sie die Funktion zum Hochladen von AEM Desktop-Apps-Ordnern
...
* **Work-in-Progress oder Creative Work-in-Progress (WIP):** Eine Phase des Asset-Lebenszyklus, in der ein Asset mehrere Änderungen durchläuft und in der Regel noch nicht für breitere Teams freigegeben werden kann.
* **Kreative Elemente:** Assets, die für ein breiteres Team freigegeben werden können oder vom Kreativteam für die Freigabe mit Marketing- oder LOB-Teams ausgewählt/genehmigt wurden.
...
* **DAM-Benutzer:** Typischer Benutzer eines DAM-Systems. Je nach Organisation können DAM-Benutzer Marketing- oder Nichtmarketingbenutzer sein, z. B. ein „Line-of-Business“(LOB)-Benutzer, Bibliothekar oder Vertriebsmitarbeiter.
...
* **Für kreative Benutzer, die in Fotoshop, InDesign oder Illustrator arbeiten:** Adobe Asset Link bietet die beste Benutzererfahrung, einschließlich der sauberen Handhabung laufender Bearbeitungen von Assets, die von AEM ausgecheckt wurden.
* **Zum Vereinfachen des Zugriffs auf Assets vom Desktop aus für beliebige allgemeine Dateiformate oder Anwendungen:** Verwenden Sie die AEM-Desktop-App.
...
* **Beachten des Volumens freigegebener Assets:** Wenn Ihr Anwendungsfall die Asset-Verteilung ist, könnten Governance und Sicherheit die wichtigsten Aspekte sein. Erwägen Sie die Verwendung von Werkzeugen, die dafür entwickelt wurden, z. B. Brand Portal.
* **Verstehen des Asset-Lebenszyklus:** Sie erfahren, wie Assets in Ihrem Unternehmen von verschiedenen Teams verarbeitet werden.
* **Vorsicht bei häufigem Speichern von Assets:** Adobe Asset Link übernimmt diese Funktion mit PS, KI, ID. Bei anderen Anwendungen führen Sie laufende Aufgaben im zugeordneten/freigegebenen Ordner nur dann aus, wenn Sie alle Änderungen in DAM benötigen.
...
In der Regel haben interne Kreativteams oder Agenturen/Kreativprofis, die an das interne Netzwerk angeschlossen sind, Zugriff auf die DAM-Instanz, einschließlich AEM-Anmeldung.

In solchen Fällen bietet die AEM-Desktop-App einfachen Zugriff auf endgültige/genehmigte Assets und ermöglicht das Speichern kreativer Assets in DAM.
...
* So erhalten Sie Zugriff auf einige Assets: Die Freigabe von AEM-Ordnern für Adobe Creative Cloud kann zusätzlich zu AEM Assets Brand Portal oder Asset Share verwendet werden. Bitte beachten Sie, dass es bestimmte Einschränkungen in Bezug auf diese Integration gibt, die in diesem Artikel ausführlicher behandelt werden.
...
1. Verwenden (platzieren) Sie genehmigte Assets in einem neuen Entwurf: Der kreative Benutzer erstellt ein neues Asset mithilfe vorhandener genehmigter Assets in Creative Cloud-Apps
...
1. Das Asset wird produziert: Dies ist ein DAM-Prozess, der je nach Organisation Tagging, Genehmigungen und die Änderung der Zugriffssteuerung umfassen kann. In dieser Phase gilt das Asset als abgeschlossen und kann von größeren Teams verwendet werden, die DAM nutzen. Es kann auch von kreativen Benutzern zum Erstellen weiterer Assets verwendet werden.

Hier finden Sie einige allgemeine Empfehlungen zur Verwaltung von Assets während dieses Prozesses:

* Verwenden Sie einen dedizierten Speicherbereich bzw. ein dediziertes Speichersystem wie den synchronisierten Ordner von Adobe Creative Cloud Assets für die WIP-Dateien: Häufige Aktualisierungen, die für DAM-Benutzer nicht relevant sind, werden am besten in einem dedizierten System anstatt in AEM Assets verarbeitet. WIP-Assets können mit der Adobe Creative Cloud-Desktop-Anwendung mit einer lokalen Festplatte synchronisiert, im lokalen Speicher gespeichert usw. werden.
...
* **** 1: Geben Sie das Asset von DAM zum Desktop frei oder öffnen Sie es direkt auf dem Desktop in der gewünschten Anwendung (z. B. Adobe Fotoshop usw.). Das Auschecken wird empfohlen, um die Datei zu sperren.
...
* To upload large/hierarchical folders, use AEM desktop app, which provides a [Folder Upload](https://helpx.adobe.com/experience-manager/desktop-app/aem-desktop-app.html#bulkupload) feature. Sie können auch hierarchische Ordnerstrukturen hochladen. Das Hochladen von Assets erfolgt im Hintergrund und ist daher nicht an eine Webbrowsersitzung gebunden.
