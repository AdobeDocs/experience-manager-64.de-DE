...
* clientlib muss auch im deaktivierten WCM-Modus enthalten sein (z. B. beim **Veröffentlichen** geladen), damit die Prädikate, Aktionen und Objektive gerendert werden können.
...
1. Um die Komponente verfügbar zu machen, müssen Sie sie bearbeiten können. To make a component editable, in CRXDE Lite, add a node `cq:editConfig` of primary type `cq:EditConfig`. Um Absätze entfernen zu können, fügen Sie eine `cq:actions`-Mehrwerteigenschaft mit `DELETE` als einzigem Wert hinzu.
...
1. Im Modus **Bearbeiten** ist die neue Komponente (z. B. **Beispielmetadaten**) jetzt im Sidekick verfügbar (in der Gruppe **Asset-Editor**). Fügen Sie die Komponente ein. Um die Metadaten speichern zu können, muss sie dem Metadatenformular hinzugefügt werden.
