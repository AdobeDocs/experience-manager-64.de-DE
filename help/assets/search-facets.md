...
1. Tippen/Klicken Sie auf das AEM-Logo und gehen Sie dann zu **[!UICONTROL Werkzeuge]** > **[!UICONTROL Allgemein]** > **[!UICONTROL Suchformulare]**.
1. From the Search Forms page, select **[!UICONTROL Assets Admin Search Rail]**, then tap  **Edit** ![aemassets_edit](assets/aemassets_edit.png).
   ...
   Suchen und wählen Sie die Admin-Suchleiste für Assets
...
1. Geben Sie im Feld **[!UICONTROL Eigenschaftsname]** einen gültigen Namen für die Metadateneigenschaft ein, die Sie mit der Vorgabe verknüpfen möchten. Es ist der Name, mit dem die Suche durchgeführt wird. Geben Sie beispielsweise `jcr:content/metadata/dc:description` oder `./jcr:content/metadata/dc:description` ein.
...
Wenn Sie z. B. „Bilder“ für Dateitypen auswählen, wird die Suchabfrage für die Assets durch Kombinieren der `value`-Eigenschaft mit einem ODER-Vorgang erstellt. Beispielsweise wird die Suchabfrage nach Bildern erstellt, indem die Ergebnisse kombiniert werden, die für *image/jpeg*, *image/gif*, *image/png*, *image/pjpeg* und *image/tiff* `jcr:content/metadata/dc:format` für die Eigenschaft übereinstimmen, die einen ODER-Vorgang verwendet.

![Die Eigenschaft Value eines Dateityps, wie in CRXDE dargestellt, wird für Suchabfragen verwendet, um zu funktionieren](assets/chlimage_1-418.png)

Die Eigenschaft Value eines Dateityps, wie in CRXDE dargestellt, wird für Suchabfragen verwendet, um zu funktionieren

Statt manuell eine Knotenstruktur für die Optionen im CRX-Repository zu erstellen, können Sie die Optionen in einer JSON-Datei definieren, indem Sie entsprechende Schlüssel-Wert-Paare angeben. Geben Sie den Pfad der JSON-Datei im Feld **[!UICONTROL Eigenschaftsname]** an. Sie können beispielsweise die Schlüssel-Wert-Paare `image/bmp`, `image/gif`, `image/jpeg` sowie `image/png` definieren und deren Werte wie in der folgenden JSON-Beispieldatei dargestellt angeben. Im Feld **[!UICONTROL Eigenschaftsname]** können Sie den CRX-Pfad für diese Datei angeben.
...
1. Wählen Sie auf der Seite **[!UICONTROL Suchformulare]** die Option **[!UICONTROL Assets-Admin-Suchleiste]** und klicken Sie dann auf das Symbol „Bearbeiten“.
1. Ziehen Sie auf der Seite **[!UICONTROL Suchformular bearbeiten]** die **[!UICONTROL Optionsvorgabe]** aus der Registerkarte **[!UICONTROL Vorgabe auswählen]** in den Hauptbereich.
1. Geben Sie auf der Registerkarte **[!UICONTROL Einstellungen]** eine Beschriftung und einen Namen für die Eigenschaft ein. Um beispielsweise Assets auf Grundlage des Formats zu suchen, geben Sie einen benutzerfreundlichen Namen für die Bezeichnung ein, z. B. **[!UICONTROL Dateityp]**. Geben Sie die Eigenschaft an, mit der die Suche im Eigenschaftenfeld ausgeführt werden soll, z. B. `jcr:content/metadata/dc:format.`.
...
1. Stellen Sie sicher, dass die **[!UICONTROL Einzelauswahl]** gelöscht wird, damit Benutzer mehrere Optionen für Dateitypen gleichzeitig auswählen können (z. B. Bilder, Dokumente, Multimedia und Archive). Wenn Sie **[!UICONTROL Einzelauswahl]** aktivieren, können Benutzer jeweils nur eine Option für Dateitypen auswählen.
...
1. Klicken Sie auf das AEM-Logo und gehen Sie dann zu **[!UICONTROL Werkzeuge]** > **[!UICONTROL Allgemein]** > Formulare **[!UICONTROL suchen]**.
...
1. Ziehen Sie auf der Seite „Suchformular bearbeiten“ eine **[!UICONTROL Vorgabe für die Eigenschaft mit mehreren Werten]** aus der Registerkarte **[!UICONTROL Vorgabe auswählen]** in den Hauptbereich.
...
1. Stellen Sie sicher, dass die **[!UICONTROL Trennzeichen-Unterstützung]** aktiviert ist. Geben Sie im Feld **[!UICONTROL Eingabe-Trennzeichen]** bestimmte Trennzeichen an, um einzelne Werte voneinander zu trennen. Standardmäßig sind Kommata als Trennzeichen angegeben. Sie können allerdings ein anderes Trennzeichen wählen.
1. In the **Description** field, enter an optional description and then tap **[!UICONTROL Done]**.
1. Navigieren Sie zum Filterbedienfeld in der Benutzeroberfläche „Assets“. Die Vorgabe für die **[!UICONTROL Eigenschaft mit mehreren Werten]** wird dem Bedienfeld hinzugefügt.
...
1. Wählen Sie **[!UICONTROL Option zur Übereinstimmung mit allen Tags anzeigen]**, um nach Assets zu suchen, die alle angegebenen Tags enthalten.
...
1. Geben Sie Tags ein, auf deren Grundlage Sie nach Assets suchen möchten, oder wählen Sie Tags aus, die in der Liste vorgeschlagen werden.
   ...
   ![AEM-bereitgestellte Empfehlung bei der Eingabe des Namens des Tags](assets/chlimage_1-419.png)
...
| [!UICONTROL Pfadbrowser] | Suchvorhersage zum Suchen nach Assets in Ordnern und Unterordnern in einem vorkonfigurierten Stammpfad | <ul><li>Platzhalter</li><li>Stammverzeichnis</li><li>Beschreibung</li></ul> |
...
| [!UICONTROL Relatives Datum] | Sucheigenschaft, um Assets basierend auf dem relativen Datum ihrer Erstellung zu suchen. Sie können beispielsweise Optionen wie „vor 2 Monaten“, „vor 3 Wochen“ usw. konfigurieren. | <ul><li>Bezeichnung</li><li>Eigenschaftsname</li><li>Relatives Datum</li></ul> |
...
![Das Symbol Sperren für eine Option auf der Seite &quot;Suchformulare&quot;gibt an, dass die Standardeinstellungen intakt und nicht angepasst sind.](assets/locked_admin_rail.png)
...
1. In the confirmation dialog, tap **[!UICONTROL Delete]** to remove the custom changes.
   ...
   Nachdem Sie die benutzerdefinierten Änderungen an Suchfacetten gelöscht haben, wird das Symbol „Sperren“ vor der **[!UICONTROL Assets-Admin-Suchleiste]** auf der Seite **[!UICONTROL Suchformulare]** erneut angezeigt.
