...
Der Knotenspeicher „Dokument“ ist die Basis der AEM-MongoMK-Implementierung. Es wird `org.apache.jackrabbit.oak.plugins.document.DocumentNodeStoreService` **PID** verwendet. Folgende Konfigurationsoptionen sind verfügbar:
...
* `repository.home`: Pfad zum Repository-Stammverzeichnis, in dem diverse Repository-bezogene Daten gespeichert werden. Binärdateien werden standardmäßig im Ordner `crx-quickstart/repository/datastore` gespeichert.

* `path`: Pfad zum Verzeichnis, unter dem Dateien gespeichert werden. Wenn angegeben, hat sie Vorrang vor dem Wert `repository.home`.
...
>[!NOTE]
>
>Bei Verwendung von AEM 6.4 mit TarMK werden die Binärdateien standardmäßig im `FileDataStore` gespeichert. Um TarMK mit dem S3-Datenspeicher zu verwenden, müssen Sie AEM mit dem Ausführungsmodus `crx3tar-nofds` Beginn ausführen. Beispiel:
...
1. Wenn AEM bereits für die Verwendung mit der Tar- oder MongoDB-Datenspeicherung konfiguriert ist, entfernen Sie alle vorhandenen Konfigurationsdateien aus dem Ordner `aem-install/crx-quickstart/install`, bevor Sie fortfahren. Die folgenden Dateien müssen entfernt werden:
...
Der Cache unterstützt asynchrone Uploads in den Datenspeicher. Die Dateien werden lokal im Cache (im Dateisystem) bereitgestellt und ein asynchroner Auftrag mit dem Upload der Datei beginnt. Die Anzahl der asynchronen Uploads ist durch die Größe des Staging-Cache begrenzt. Die Größe des Staging-Cache wird mithilfe des `stagingSplitPercentage`-Parameters konfiguriert. Dieser Parameter definiert den Prozentsatz der Cachegröße, der für den Staging-Cache verwendet werden soll. Außerdem wird der für Downloads verfügbare Cache-Prozentsatz berechnet als **(100 - `stagingSplitPercentage`) &amp;ast;`cacheSize`**.
...
>[!NOTE]
>
>Bei Verwendung von AEM 6.4 mit TarMK werden die Binärdateien standardmäßig im FileDataStore gespeichert. Um TarMK mit dem Azurblauen DataStore zu verwenden, müssen Sie AEM mit dem Ausführungsmodus `crx3tar-nofds` Beginn ausführen. Beispiel:
