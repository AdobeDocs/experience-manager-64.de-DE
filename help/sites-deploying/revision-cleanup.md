...
**Hinweis**: [Siehe Video](https://helpx.adobe.com/experience-manager/kt/platform-repository/using/revision-cleanup-technical-video-use.html) für eine Einführung und die Verwendung der Online-Revisionsbereinigung.
...
Darüber hinaus können Sie auch die [offizielle Oak-Dokumentation.](https://jackrabbit.apache.org/oak/docs/nodestore/segment/overview.html)
...
1. Navigieren Sie im AEM-Hauptfenster zu **Tools - Vorgänge - Dashboard - Wartung** oder verweisen Sie Ihren Browser auf: `https://serveraddress:serverport/libs/granite/operations/content/maintenance.html`

   ![chlimage_1-90](assets/chlimage_1-90.png)

1. Bewegen **Tägliches Wartungsfenster** und klicken Sie auf **Einstellungen** Symbol.
...
1. Navigieren Sie zu **Tools - Vorgänge - Dashboard - Wartung** oder direkt zu `https://serveraddress:serverport/libs/granite/operations/content/maintenance.html`
1. Klicken Sie auf **Tägliches Wartungsfenster**.
1. Bewegen Sie den Mauszeiger über die **Revisionsbereinigung** Symbol.
...
In der Standardkonfiguration wird die Tail-Komprimierung an Wochentagen und die vollständige Komprimierung an Sonntagen ausgeführt. Die Standardkonfiguration kann mithilfe des neuen Konfigurationswerts geändert werden `full.gc.days` des `RevisionCleanupTask` [Wartungsaufgabe](/help/sites-deploying/revision-cleanup.md#how-to-run-online-revision-cleanup).

Wenn Sie die `full.gc.days` Beachten Sie, dass die vollständige Komprimierung während der in der Wert definierten Tage ausgeführt wird und die Tail-Komprimierung während der Tage ausgeführt wird, die nicht im Wert definiert sind. Wenn Sie beispielsweise die Ausführung einer vollständigen Komprimierung am Sonntag konfigurieren, wird die Tail-Komprimierung von Montag bis Samstag ausgeführt. Wenn Sie hingegen die Ausführung einer vollständigen Komprimierung an allen Tagen der Woche konfigurieren, wird die Tail-Komprimierung gar nicht ausgeführt.

Berücksichtigen Sie auch Folgendes:

* **Tail-Komprimierung** ist weniger effektiv und hat weniger Auswirkungen auf den normalen Systembetrieb. Sie sollte daher an den Werktagen ausgeführt werden.
* **Vollständige Komprimierung** ist effektiver, hat aber auch größere Auswirkungen auf den normalen Systembetrieb. Sie sollte daher außerhalb der Arbeitstage verwendet werden.
...
* Die `RevisionCleanupTaskHealthCheck` gibt den allgemeinen Gesundheitsstatus der Online-Revisionsbereinigung an. Er funktioniert wie in AEM 6.3 und unterscheidet nicht zwischen der vollständigen und der Tail-Komprimierung.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>Das Persistenzformat von TarMK ändert sich mit AEM 6.4. Für diese Änderungen ist keine proaktive Migration erforderlich. Vorhandene Repositorys durchlaufen eine parallele Migration, die für den Benutzer transparent ist. Der Migrationsprozess wird initiiert, wenn AEM 6.4 (oder zugehörige Tools) zum ersten Mal auf das Repository zugreifen.</p> <p><strong>Nachdem die Migration zum Persistenzformat AEM 6.4 initiiert wurde, kann das Repository nicht mehr in das Persistenzformat AEM 6.3 zurückgesetzt werden.</strong></p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Die Migrationsleistung kann erheblich verbessert werden, wenn <a href="/help/sites-deploying/revision-cleanup.md#how-to-run-offline-revision-cleanup">Offline-Revisionsbereinigung</a> vor der Migration ausgeführt wird. Allen Kunden wird empfohlen, diese als Voraussetzung für die Aktualisierung auszuführen. Im Allgemeinen sollte die Dauer der Migration der Dauer der Offline-Revisionsbereinigung ähneln, vorausgesetzt, die Offline-Revisionsbereinigung wurde vor der Migration ausgeführt.</td> 
   <td> </td> 
  </tr> 
 </tbody> 
</table>
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Siehe <a href="/help/sites-deploying/revision-cleanup.md#how-to-run-online-revision-cleanup">Ausführen der Online-Revisionsbereinigung</a> Abschnitt. </td> 
   <td> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><p>Die Revisionsbereinigung ermittelt in einer Schätzungsphase, ob eine Bereinigung notwendig ist. Bei der Schätzung wird die aktuelle Repository-Größe mit der Größe nach der letzten Komprimierung verglichen. Falls die Größe den konfigurierten Deltawert übersteigt, erfolgt eine Bereinigung. Der Delta-Formatbereich ist auf 1 GB eingestellt. Dies bedeutet im Grunde, dass die neue Revisionsbereinigungs-Iteration übersprungen wird, wenn die Repository-Größe seit dem letzten Bereinigungsvorgang nicht um 1 GB gewachsen ist. </p> <p>Unten sehen Sie die für die Schätzungsphase relevanten Protokolleinträge:</p> 
    <ul> 
     ...
     <li>Revisionsbereinigung wird <strong>not</strong> run: <em>Delta-Größe ist N% oder N/N (N/N Byte), daher wird die Komprimierung für jetzt übersprungen</em></li> 
    </ul> </td> 
   <td> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><p>Wenn im System gleichzeitige Schreibvorgänge stattfinden, benötigt die Online-Revisionsbereinigung möglicherweise einen exklusiven Schreibzugriff, um die Änderungen am Ende eines Komprimierungszyklus zu committen. Das System wird <strong>forceCompact-Modus</strong>, wie im Abschnitt <a href="https://jackrabbit.apache.org/oak/docs/nodestore/segment/overview.html" target="_blank">Oak-Dokumentation</a>. Bei der erzwungenen Komprimierung wird eine exklusive Schreibsperre angewendet, damit die Änderungen ohne störende gleichzeitige Schreibvorgänge gespeichert werden können. Um die Auswirkungen auf die Reaktionszeiten zu begrenzen, kann ein Zeitlimitwert festgelegt werden. Dieser Wert beträgt standardmäßig 1 Minute. Falls also die erzwungene Komprimierung nicht innerhalb 1 Minute abgeschlossen ist, wird die Komprimierung zugunsten gleichzeitiger Speichervorgänge abgebrochen.</p> <p>Die Dauer der erzwungenen Komprimierung hängt von folgenden Faktoren ab:</p> 
    <ul> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>Sie können überprüfen, ob die Online-Revisionsbereinigung erfolgreich abgeschlossen wurde, indem Sie die Protokolle überprüfen.</p> <p>Beispiel: "<code>TarMK GC #{}: compaction completed in {} ({} ms), after {} cycles</code>" bedeutet, dass der Komprimierungsschritt erfolgreich abgeschlossen wurde, es sei denn, der Nachricht vorangestellt ist "<code>TarMK GC #{}: compaction gave up compacting concurrent commits after {} cycles</code>", was bedeutet, dass es zu viel gleichzeitige Last gab.</p> <p>Dementsprechend gibt es eine Meldung "<code>TarMK GC #{}: cleanup completed in {} ({} ms</code>" für den erfolgreichen Abschluss des Bereinigungsschritts.</p> </td> 
   <td><p> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 
    ...
    </ul> <p>Siehe auch <a href="/help/sites-deploying/revision-cleanup.md#troubleshooting-based-on-error-messages">Fehlerbehebung anhand von Fehlermeldungen</a> unten.</p> </td> 
   <td> </td> 
  </tr> 
  <tr> 
   ...
   <td>Am Ende des Bereinigungszyklus wird im Protokoll eine Meldung angezeigt: "<code>TarMK GC #3: cleanup completed</code>", das die Größe des Repositorys und die Menge des wiederhergestellten Speicherplatzes enthält.</td> 
   <td> </td> 
  </tr> 
  <tr> 
   ...
   <td><p>Nach der Online-Revisionsbereinigung ist keine Repository-Integritätsprüfung erforderlich. </p> <p>Sie können jedoch die folgenden Aktionen ausführen, um den Repository-Status nach der Bereinigung zu überprüfen:</p> 
    <ul> 
     <li>Ein Repository <a href="/help/sites-deploying/consistency-check.md" target="_blank">traversal</a></li> 
     <li>Verwenden Sie das oak-run-Tool nach Abschluss des Bereinigungsprozesses, um nach Inkonsistenzen zu suchen. Weitere Informationen hierzu finden Sie im Abschnitt <a href="https://github.com/apache/jackrabbit-oak/blob/trunk/oak-doc/src/site/markdown/nodestore/segment/overview.md#check" target="_blank">Apache-Dokumentation.</a> Sie müssen das Tool nicht herunterfahren, AEM Sie es ausführen.</li> 
    </ul> </td> 
   <td> </td> 
  </tr> 
  <tr> 
   ...
   <td>Fehlerbedingungen werden durch WARN- oder ERROR-Protokollmeldungen gekennzeichnet, die mit "TarMK GC"beginnen. Siehe auch <a href="/help/sites-deploying/revision-cleanup.md#troubleshooting-based-on-error-messages">Fehlerbehebung anhand von Fehlermeldungen</a> unten.</td> 
   <td> </td> 
  </tr> 
  <tr> 
   ...
   <td><p>Die Konsistenzprüfung der Revisionsbereinigung ist Teil der <a href="/help/sites-administering/operations-dashboard.md#health-reports" target="_blank">Vorgangs-Dashboard</a>.<br /> </p> <p>Der Status lautet <strong>GRÜNE</strong> wenn die letzte Ausführung der Online-Revisionsbereinigung erfolgreich abgeschlossen wurde.</p> <p>Es wird <strong>GELB</strong> wenn die Wartungsaufgabe für die Online-Revisionsbereinigung einmal abgebrochen wurde.<br /> </p> <p>Es wird <strong>RED</strong> wenn die Wartungsaufgabe für die Online-Revisionsbereinigung dreimal hintereinander abgebrochen wurde. <strong>In diesem Fall ist eine manuelle Interaktion erforderlich</strong> oder die Online-Revisionsbereinigung wird wahrscheinlich erneut fehlschlagen. Weitere Informationen finden Sie im Abschnitt <a href="/help/sites-deploying/revision-cleanup.md#troubleshooting-online-revision-cleanup">Fehlerbehebung</a> unten.<br /> </p> <p>Beachten Sie, dass der Status der Konsistenzprüfung nach jedem Systemneustart zurückgesetzt wird. Für eine neu gestartete Instanz wird der Status der Konsistenzprüfung der Revisionsbereinigung grün angezeigt. Sie können externe Überwachungstools verwenden, um Daten über die AEM-Laufzeit hinaus aufzubewahren. Siehe <a href="/help/sites-administering/operations-dashboard.md#monitoring-with-nagios">die AEM-Dokumentation für das Anhängen von Konsistenzprüfungen an Nagios als Beispiel für ein externes Überwachungstool</a>.</p> </td> 
   <td> </td> 
  </tr> 
  <tr> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><strong>Was verursacht <code>SegmentNotFoundException</code> Instanzen, die in der <code>error.log</code> und wie kann ich mich erholen?</strong></td> 
   <td><p>A <code>SegmentNotFoundException</code> wird vom TarMK protokolliert, wenn versucht wird, auf eine Speichereinheit (ein Segment) zuzugreifen, die nicht gefunden werden kann. Es gibt drei Situationen, die diesen Fehler verursachen können:</p> 
    <ol> 
     ...
     <li>Ein externes Ereignis hat die Beschädigung der Daten auf der Festplatte verursacht. Hierbei kann es sich um einen Datenträgerfehler, ungenügenden Festplattenspeicher oder eine unbeabsichtigte Änderung der erforderlichen Datendateien handeln. In diesem Fall müssen Sie die Instanz in den Offline-Modus versetzen und den Fehler mithilfe der oak-run-Prüfung beheben. Weitere Informationen zur Durchführung der Oak-Run-Prüfung finden Sie in den folgenden <a href="https://github.com/apache/jackrabbit-oak/blob/trunk/oak-doc/src/site/markdown/nodestore/segment/overview.md#check" target="_blank">Apache-Dokumentation</a>.</li> 
     <li>Alle anderen Ereignisse sollten über die <a href="https://helpx.adobe.com/de/marketing-cloud/contact-support.html" target="_blank">Adobe-Kundenunterstützung</a>.</li> 
    </ol> </td> 
   <td> </td> 
  </tr> 
 </tbody> 
...
>
>* Für Oak-Versionen **1.0.0 bis 1.0.11** oder **1.1.0 bis 1.1.6**, verwenden Sie Oak-Run-Version** 1.0.11**
>
...
Adobe bietet ein Tool mit dem Namen **Oak-run** für die Durchführung der Revisionsbereinigung. Es kann unter folgender URL heruntergeladen werden:
...
Tipps zur Leistungssteigerung des Bereinigungsprozesses finden Sie unter [Erhöhen der Leistung der Offline-Revisionsbereinigung](/help/sites-deploying/revision-cleanup.md#increasing-the-performance-of-offline-revision-cleanup).
...
* **-Dtar.PersistCompactionMap.** Setzen Sie diesen Parameter auf &quot;true&quot;, um Speicherplatz anstelle des Heap-Speichers für die Persistenz von Komprimierungskarten zu verwenden. Erfordert das Tool &quot;oak-run&quot; **Versionen 1.4** und höher. Weitere Informationen finden Sie unter Frage 3 im Abschnitt [Häufig gestellte Fragen zur Offline-Revisionsbereinigung. ](/help/sites-deploying/revision-cleanup.md#offline-revision-cleanup-frequently-asked-questions) **Dieser Parameter wurde in Oak Version 1.6 entfernt und hat keine Auswirkung.** 

* **--force.** Erzwingen Sie die Komprimierung und ignorieren Sie eine nicht übereinstimmende Segmentspeicherversion.

>[!CAUTION]
>
>Verwenden der `--force` aktualisiert den Segmentspeicher auf die neueste Version, die mit älteren Oak-Versionen inkompatibel ist. Beachten Sie außerdem, dass kein Downgrade möglich ist. Generell sollten Sie diese Parameter vorsichtig verwenden und nur, wenn Sie wirklich wissen, wie sie verwendet werden.
...
1. Öffnen Sie die JMX-Konsole, indem Sie [http://localhost:4502/system/console/jmx](http://localhost:4502/system/console/jmx)
1. Klicken Sie auf **RevisionGarbageCollection** MBean.
1. Klicken Sie im nächsten Fenster auf **startRevisionGC()** und dann **Aufrufen** , um den Auftrag zur Revisionsbereinigung zu starten.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Wenn die Revisionsaufgabe nicht innerhalb von 8 Stunden abgeschlossen wird und die <a href="/help/sites-administering/operations-dashboard.md#diagnosis-tools" target="_blank">Thread-Sicherheitskopien</a> erkennen, dass der Haupt-Hotspot <code>InMemoryCompactionMap.findEntry</code>verwenden Sie den folgenden Parameter mit dem Tool oak-run . <strong>Versionen 1.4 </strong>oder höher: <code>-Dtar.PersistCompactionMap=true</code>. Beachten Sie, dass <code>-Dtar.PersistCompactionMap</code> wurde in Oak Version 1.6 entfernt.</td> 
  </tr> 
 </tbody> 
</table>
