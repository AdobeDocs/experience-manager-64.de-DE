...
# Beginn zur Forms Service API {#forms-service-api-quick-starts}
...
Im folgenden Codebeispiel wird ein interaktives PDF-Formular mit dem Namen *Loan.xdp* an einen Client-Webbrowser gerendert. Eine Datei wird an das Formular angehängt. Beachten Sie, dass der Formularentwurf Teil einer Anwendung ist und mithilfe des Inhaltsstamm-URI-Werts referenziert wird `repository:///`. (Siehe [Rendern von interaktiven PDF forms](/help/forms/developing/rendering-forms.md#rendering-interactive-pdf-forms).)
...
Im folgenden Codebeispiel wird ein Formular mit dem Namen *Loan.xdp *auf dem Client mit der Java-API des Forms-Dienstes gerendert. Beachten Sie, dass der Formularentwurf Teil einer Anwendung ist und mithilfe des Inhaltsstamm-URI-Werts referenziert wird `repository:///`. (Siehe [Rendern von Forms auf dem Client](/help/forms/developing/rendering-forms.md#rendering-forms-at-the-client).)
...
## Quick Beginn (SOAP-Modus): Wiedergabe eines HTML-Formulars, das eine CSS-Datei mit der Java-API verwendet {#quick-start-soap-mode-rendering-an-html-form-that-uses-a-css-file-using-the-java-api}

Im folgenden Codebeispiel wird ein HTML-Formular mit der Forms-Dienst-Client-API gerendert. Der Name der benutzerdefinierten CSS-Datei, auf die verwiesen wird, lautet *custom.css*. (Siehe [Rendern von HTML Forms mit benutzerdefinierten CSS-Dateien](/help/forms/developing/rendering-forms.md#rendering-html-forms-using-custom-css-files).)
...
## Quick Beginn (SOAP-Modus): Umgang mit PDF forms, die als XML mit der Java-API gesendet werden {#quick-start-soap-mode-handling-pdf-forms-submitted-as-xml-using-the-java-api}

Im folgenden Codebeispiel wird ein Formular verarbeitet, das als XML gesendet wird. Der an die `processFormSubmission` Methode übergebene Inhaltstypwert ist `CONTENT_TYPE=text/xml`. Die Werte, die den Feldern mit den Namen `mortgageAmount`, `lastName`und `firstName` werden angezeigt. In diesem Beginn `getNodeText` wird eine benutzerdefinierte Methode verwendet. Es akzeptiert eine `org.w3c.dom.Document` Instanz und einen Zeichenfolgenwert, der den Knotennamen angibt. Diese Methode gibt einen Zeichenfolgenwert zurück, der den Wert der Node darstellt. (Siehe [Umgang mit übermittelten Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms).)
...
## Quick Beginn (SOAP-Modus): Umgang mit PDF forms, die als PDF mit der Java-API gesendet wurden {#quick-start-soap-mode-handling-pdf-forms-submitted-as-pdf-using-the-java-api}

Im folgenden Codebeispiel wird ein Formular behandelt, das als PDF-Daten gesendet wird. Der an die `processFormSubmission` Methode übergebene Inhaltstypwert ist `CONTENT_TYPE=application/pdf`. Das gesendete Formular wird als PDF-Datei mit dem Namen *tempPDF.pdf* gespeichert. Da das Formular als PDF gesendet wird, können auch Dateianlagen abgerufen werden. Alle Dateianlagen werden als JPEG-Dateien gespeichert. (Siehe [Umgang mit übermittelten Forms](/help/forms/developing/rendering-forms.md#handling-submitted-forms).)
...
* `renderPOForm`: Verwendet die Java-API des Forms-Dienstes zum Rendern eines dynamischen Bestellformulars. Das `com.adobe.idp.Document` von der `convertDataSource` Methode zurückgegebene Objekt wird zum Vorausfüllen des Formulars verwendet.
   ...
   Alle diese Methoden werden von der Java-Servlet- `doPost` Methode aufgerufen. (Siehe [Vorausfüllen von Forms mit flexiblen Layouts](/help/forms/developing/rendering-forms.md#prepopulating-forms-with-flowable-layouts).)
...
## Quick Beginn (SOAP-Modus): Verarbeiten eines Formulars mit einem Berechnungsskript mithilfe der Java-API {#quick-start-soap-mode-handling-a-form-containing-a-calculation-script-using-the-java-api}
...
Im folgenden Java Quick Beginn wird ein interaktives PDF-Formular wiedergegeben, das auf einem Formularentwurf mit dem Namen *Loan.xdp* basiert. Beachten Sie, dass der Formularentwurf zum Ausfüllen eines `com.adobe.idp.Document` Objekts mit dem Namen* inputXDP* verwendet wird. (Siehe [Rendern von Forms nach Wert](/help/forms/developing/rendering-forms.md#rendering-forms-by-value).)
