...
fscJS ist das Java-Skript, das mit jedem Knoten verknüpft ist. Es ist erforderlich, einen für die `div#fscmenu` Knoten und optional für `ul#fscmenuItem` Knoten. Die JS-Dateien implementieren die Kernfunktionalität der Symbolleiste und die Standarddateien funktionieren.
...
Sie können auch die absoluten Speicherorte dieser JS- und CSS-Dateien angeben, z. B. https://www.mycompany.com/scripts/misc/fscmenu.js. In diesem Fall müssen Sie die `FSToolBarURI` Keyword.

>[!NOTE]
>
>Es wird nicht empfohlen, die Referenzierung dieser Dateien zu verändern. Das heißt, alle URIs sollten referenziert werden, indem Sie entweder `FSToolBarURI` oder eine absolute Position.

Sie können die JS- und CSS-Dateien abrufen, indem Sie die adobe-forms-&lt;appserver>.ear-Datei. Öffnen Sie in dieser Datei adobe-forms-res.war. Alle diese Dateien befinden sich in der WAR-Datei. Die adobe-forms-&lt;appserver>Die .ear-Datei befindet sich im Installationsordner für AEM Formulare (C:\ is the installation directory). Sie können die adobe-forms-&lt;appserver>.ear mit einem Dateiextraktions-Tool wie WinRAR.
...
* Ändern Sie die Werte von `fscJS`, `fscCSS`, `fscVCSS`, `fscIECSS` -Attribute (in der Datei &quot;fscmenu.xml&quot;), um die benutzerdefinierten Speicherorte der referenzierten Dateien mithilfe einer der in diesem Abschnitt beschriebenen Methoden (z. B. `fscJS="FSToolBarURI/scripts/fscmenu.js"`).
...
* Die JS- und CSS-Dateien, die die `div#fscmenu` -Knotenanforderungen sind für die Symbolleistenfunktion unverzichtbar. Individuum `ul#fscmenuItem` -Knoten können unterstützende JS- oder CSS-Dateien enthalten oder nicht.
...
Geben Sie außerdem einen gültigen Gebietsschemawert an, indem Sie die `HTMLRenderSpec` -Objekt `setLocale` -Methode verwenden und einen string -Wert übergeben, der den Gebietsschema-Wert angibt. Übergeben Sie beispielsweise `fr_FR` um Französisch anzugeben. Der Forms-Dienst ist mit lokalisierten Symbolleisten gebündelt.

>[!NOTE]
>
>Bevor Sie ein HTML-Formular rendern, das eine benutzerdefinierte Symbolleiste verwendet, müssen Sie wissen, wie HTML-Formulare wiedergegeben werden. (Siehe [Rendern von Forms als HTML](/help/forms/developing/rendering-forms-html.md).
...
Um ein HTML-Formular mit einer benutzerdefinierten Symbolleiste wiederzugeben, führen Sie die folgenden Schritte aus:
...
Wenn der Forms-Dienst ein HTML-Formular rendert, wird ein Formulardatenstream zurückgegeben, den Sie in den Client-Webbrowser schreiben müssen, damit das HTML-Formular für Benutzer sichtbar wird.
...
Rendern Sie ein HTML-Formular mit einer benutzerdefinierten Symbolleiste mithilfe der Forms Service API (Java):
...
1. Referenzieren einer benutzerdefinierten XML-Datei fscmenu
   ...
   * Geben Sie den Speicherort der XML-Datei fscmenu an, indem Sie die `HTMLRenderSpec` -Objekt `setToolbarURI` -Methode verwenden und einen string -Wert übergeben, der den URI-Speicherort der XML-Datei angibt.
   * Falls zutreffend, legen Sie den Gebietsschemawert fest, indem Sie die `HTMLRenderSpec` -Objekt `setLocale` -Methode verwenden und einen string -Wert übergeben, der den Gebietsschema-Wert angibt. Der Standardwert ist Englisch.
...
1. Rendern eines HTML-Formulars
   ...
   Rufen Sie die `FormsServiceClient` -Objekt `renderHTMLForm` -Methode verwenden und die folgenden Werte übergeben:
   ...
   * A `java.util.HashMap` -Objekt, das Dateianlagen speichert. Dies ist ein optionaler Parameter, den Sie `null` , wenn Sie keine Dateien an das Formular anhängen möchten.
   ...
   Die `renderHTMLForm` -Methode gibt eine `FormsResult` -Objekt, das einen Formulardatenstrom enthält, der in den Client-Webbrowser geschrieben werden muss.
...
1. Schreiben Sie den Formulardaten-Stream in den Client-Webbrowser
   ...
   * Erstellen Sie eine `javax.servlet.ServletOutputStream` -Objekt, das zum Schreiben des Formulardatenstreams in den Client-Webbrowser durch Aufrufen der `javax.servlet.http.HttpServletResponse` -Objekt `getOutputStream` -Methode.
...
Rendern Sie ein HTML-Formular mit einer benutzerdefinierten Symbolleiste mithilfe der Forms Service-API (Webdienst):
...
1. Rendern eines HTML-Formulars
   ...
   Rufen Sie die `FormsService` -Objekt `renderHTMLForm` -Methode verwenden und die folgenden Werte übergeben:
   ...
   * A `URLSpec` -Objekt, das URI-Werte speichert, die zum Rendern eines HTML-Formulars erforderlich sind.
   * A `java.util.HashMap` -Objekt, das Dateianlagen speichert. Dieser Parameter ist optional und Sie können `null` wenn Sie keine Dateien an das Formular anhängen möchten.
   * Ein leeres `com.adobe.idp.services.holders.BLOBHolder` -Objekt, das von der `renderHTMLForm` -Methode. Dieser Parameterwert speichert das wiedergegebene Formular.
   * Ein leeres `com.adobe.idp.services.holders.BLOBHolder` -Objekt, das von der `renderHTMLForm` -Methode. Dieser Parameter speichert die XML-Ausgabedaten.
   * Ein leeres `javax.xml.rpc.holders.LongHolder` -Objekt, das von der `renderHTMLForm` -Methode. Dieses Argument speichert die Anzahl der Seiten im Formular.
   * Ein leeres `javax.xml.rpc.holders.StringHolder` -Objekt, das von der `renderHTMLForm` -Methode. Dieses Argument speichert den Gebietsschemawert.
   * Ein leeres `javax.xml.rpc.holders.StringHolder` -Objekt, das von der `renderHTMLForm` -Methode. Dieses Argument speichert den verwendeten HTML-Rendering-Wert.
   * Ein leeres `com.adobe.idp.services.holders.FormsResultHolder` -Objekt, das die Ergebnisse dieses Vorgangs enthält.

   Die `renderHTMLForm` -Methode füllt die `com.adobe.idp.services.holders.FormsResultHolder` -Objekt, das als letzter Argumentwert mit einem Formulardatenstream übergeben wird, der in den Client-Webbrowser geschrieben werden muss.
