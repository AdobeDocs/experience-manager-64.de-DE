---
title: Vorbereiten von AEM Forms für Sicherung
seo-title: Vorbereiten von AEM Forms für Sicherung
description: 'null'
...
Mit dem Sicherungs- und Wiederherstellungsdienst können Sie AEM Forms in den *Sicherungsmodus* versetzen, der die Ausführung von Hot Backups ermöglicht. Der Sicherungs- und Wiederherstellungsdienst führt keine Sicherung von AEM Forms durch oder stellt Ihr System wieder her. Stattdessen versetzt es Ihren Server in einen Status für konsistente und zuverlässige Backups, während der Server weiterhin ausgeführt werden kann. Sie sind für die Aktionen zum Sichern des globalen Dokumentenspeichers (GDS) und der mit dem Formularserver verbundenen Datenbank verantwortlich. Der globale Dokumentenspeicher ist ein Ordner zum Speichern von Dateien, die in einem Prozess mit langer Lebensdauer verwendet werden.
...
>[!NOTE]
>
>Weitere Informationen zum Durchführen von Sicherungen für AEM Forms finden Sie in der [Administration-Hilfe](https://www.adobe.com/go/learn_aemforms_admin_63).
...
Schließen Sie die erforderlichen Dateien in Ihr Entwicklungsprojekt ein. Diese Dateien sind wichtig, damit sie in Ihr Projekt aufgenommen werden können, damit der Code ordnungsgemäß kompiliert und die Sicherungs- und Wiederherstellungs-Dienst-API verwendet wird.
...
Nach erfolgreichem Starten des Sicherungsmodus können Sie eine Sicherung des globalen Dokumentenspeichers (GDS) und der Datenbank durchführen, mit der der Formularserver verbunden ist. Dieser Schritt ist für Ihr Unternehmen spezifisch, da Sie diesen Schritt manuell durchführen können oder andere Werkzeuge zum Durchführen des Sicherungsverfahrens ausführen können.
...
1. Sicherungsmodus aktivieren
   ...
   * Ein `Boolean` Wert, der angibt, ob der kontinuierliche Sicherungsmodus aktiviert ist. Der Wert von `True` gibt an, im kontinuierlichen Sicherungsmodus zu sein. Im kontinuierlichen Sicherungsmodus wird der Wert ignoriert, den Sie für die Anzahl der Minuten angeben, die im Sicherungsmodus verbleiben sollen.
...
1. Abrufen von Informationen zur Sicherungsmodussitzung auf dem Server
   ...
   Rufen Sie Informationen mit dem `BackupModeEntryResult` Objekt ab, das nach dem Aufrufen der `enterBackupMode` Methode zurückgegeben wird. Die Informationen, die Sie nach dem Starten des Sicherungsmodus abrufen können, können für die Integration in Ihre Sicherungsverfahren nützlich sein. Beispielsweise können die Beschriftung, die Sicherungs-ID und die Startzeit als Eingabe für Dateinamen für Ihr Sicherungsverfahren nützlich sein.

1. Sicherung des globalen Dokumentenspeichers und der Datenbank durchführen

   Sichern Sie den globalen Dokumentenspeicher (GDS) und die Datenbank, mit der Ihr Formularserver verbunden ist. Die Aktionen zum Durchführen der Sicherung sind nicht Teil des AEM Forms SDK und können sogar manuelle Schritte für die jeweiligen Sicherungsverfahren in Ihrem Unternehmen enthalten.
...
Sie können den Sicherungsmodus deaktivieren, damit der Formularserver das Bereinigen von Dateien aus dem globalen Dokumentenspeicher (GDS) auf dem Formularserver fortsetzt.
...
Lassen Sie den Sicherungsmodus beendet, um das normale Bereinigen von Dateien aus dem globalen Dokumentenspeicher (GDS) fortzusetzen. Bevor Sie den Sicherungsmodus beenden, sollten Sie sicherstellen, dass die Sicherungsverfahren abgeschlossen sind.
