...
Folgende Aufgaben können Sie mit dem Convert PDF-Dienst ausführen:

* Konvertieren von PDF-Dokumenten in PostScript – 
* Konvertieren von PDF-Dokumenten in Bildformate

   ***Hinweis **: Weitere Informationen zum Convert PDF-Dienst finden Sie unter[Dienste-Referenz für AEM Forms](https://www.adobe.com/go/learn_aemforms_services_63).*
...
1. Verweisen Sie auf das zu konvertierende PDF-Dokument in eine PostScript-Datei.
...
Die generierte PostScript-Datei spiegelt in der Regel die Größe des PDF-Eingabedokuments wider. Wenn Sie die `ShrinkToFit` Option auswählen (mit der die Ausgabe der PostScript-Datei auf die Seite angepasst wird), wird kein Unterschied zwischen dem PDF-Eingabedokument und der generierten PostScript-Datei angezeigt. Die `ShrinkToFit` Option wird nur aktiviert, wenn Sie die Druckoption auf einer kleineren Seitengröße als das PDF-Eingabedokument auswählen. Um eine kleinere Seitengröße auszuwählen, definieren Sie die `PageSize` Option. Darüber hinaus wird empfohlen, die `RotateAndCenter` Option so festzulegen, dass die richtige PostScript-Ausgabe abgerufen `true` wird.

Wenn Sie die `ExpandToFit` Option (mit der die Ausgabe der PostScript-Datei an die Seite angepasst wird) auswählen, wird sie nur dann wirksam, wenn Sie die Druckausgabe auf einer größeren Seitengröße als das PDF-Eingabedokument vornehmen. Um ein größeres Seitenformat auszuwählen, definieren Sie die `PageSize` Option. Darüber hinaus wird empfohlen, die `RotateAndCenter` Option so festzulegen, dass die richtige PostScript-Ausgabe abgerufen `true` wird.
...
Nachdem Sie den Dienstclient erstellt und Laufzeitoptionen festgelegt haben, können Sie den PostScript-Konvertierungsvorgang aufrufen. Dieser Vorgang erfordert Informationen zum zu konvertierenden Dokument, einschließlich der bevorzugten PostScript-Ebene für das Zieldokument.
...
[Konvertieren eines PDF-Dokuments mit der Webdienst-API in PS](converting-pdf-postscript-image-files.md#convert-a-pdf-document-to-ps-using-the-web-service-api)

[Einbeziehung von AEM Forms Java-Bibliotheksdateien](/help/forms/developing/invoking-aem-forms-using-java.md#including-aem-forms-java-library-files)

[Verbindungseigenschaften festlegen](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)

[PDF-Dienst-API-Kurzanleitungen konvertieren](/help/forms/developing/convert-pdf-service-java-api.md#convert-pdf-service-java-api-quick-start-soap)

### Konvertieren eines PDF-Dokuments mit der Java-API in PS {#convert-a-pdf-document-to-ps-using-the-java-api}

Konvertieren eines PDF-Dokuments mit der Convert PDF Service API (Java) in PostScript:

1. Schließen Sie Projektdateien ein.

   Schließen Sie Client-JAR-Dateien wie &quot;adobe-convertpdf-client.jar&quot;im Klassenpfad Ihres Java-Projekts ein.
...
[Kurzanleitung (SOAP-Modus): Konvertieren eines PDF-Dokuments in PostScript mit der Java-API](/help/forms/developing/convert-pdf-service-java-api.md#quick-start-soap-mode-converting-a-pdf-document-to-postscript-using-the-java-api)

[Einbeziehung von AEM Forms Java-Bibliotheksdateien](/help/forms/developing/invoking-aem-forms-using-java.md#including-aem-forms-java-library-files)

[Verbindungseigenschaften festlegen](/help/forms/developing/invoking-aem-forms-using-java.md#setting-connection-properties)

### Konvertieren eines PDF-Dokuments mit der Webdienst-API in PS {#convert-a-pdf-document-to-ps-using-the-web-service-api}
...
1. Legen Sie Laufzeitoptionen für Konversionen fest.
   ...
   * Create a `ToPSOptionsSpec` object by invoking its constructor.
   * Legen Sie Laufzeitoptionen fest, indem Sie dem Datenmember des `ToPSOptionsSpec` Objekts einen Wert zuweisen. Um beispielsweise die erstellte PostScript-Ebene zu definieren, weisen Sie dem `PSLevel` Datenmember des Objekts einen `ToPSOptionsSpec` `psLevel` Aufzählungswert zu.
...
1. Speichern Sie die PostScript-Datei.
   ...
   * Erstellen Sie ein Bytearray, das den Dateninhalt des `BLOB` Objekts speichert, das von der `encryptPDFUsingPassword` Methode zurückgegeben wurde. Füllen Sie das Bytearray, indem Sie den Wert des `BLOB` Objektfelds abrufen `MTOM` .
...
Mit dem Convert PDF-Dienst können Sie PDF-Dokumente programmgesteuert in Bildformate konvertieren, darunter JPEG, JPEG 2000, TIFF und PNG. Durch Konvertieren eines PDF-Dokuments in eine Bilddatei können Sie das PDF-Dokument als Bilddatei verwenden. Sie können das Bild beispielsweise in einem Enterprise Content Management-System zur Speicherung ablegen.

Beim Konvertieren eines PDF-Dokuments in ein Bild erstellt der Convert PDF-Dienst ein separates Bild für jede Seite im Dokument. Das heißt, wenn das Dokument 20 Seiten umfasst, erstellt der Convert PDF-Dienst 20 Bilddateien. Beim Konvertieren eines PDF-Dokuments in ein Bildformat können Sie für jede Seite im PDF-Dokument oder für das gesamte PDF-Dokument einzelne Bilder erstellen.
...
1. Legen Sie Laufzeitoptionen fest.
   ...
   * Rufen Sie nach Bedarf Methoden auf, die zu diesem Objekt gehören. Legen Sie beispielsweise den Bildtyp fest, indem Sie die `setImageConvertFormat` Methode aufrufen und einen `ImageConvertFormat` Enum-Wert übergeben, der den Formattyp angibt.
   >[!NOTE]
   >
   >Das Festlegen des `ImageConvertFormat` Aufzählungswerts ist obligatorisch.
...
[Kurzanleitung (SOAP-Modus): Konvertieren eines PDF-Dokuments in JPEG-Dateien mit der Java-API](/help/forms/developing/convert-pdf-service-java-api.md#quick-start-soap-mode-converting-a-pdf-document-to-jpeg-files-using-the-java-api)

### Konvertieren eines PDF-Dokuments in Bilddateien mithilfe der Web Service API {#convert-a-pdf-document-to-image-files-using-the-web-service-api}

Konvertieren eines PDF-Dokuments mithilfe der Convert PDF Service API (Webdienst) in ein Bildformat:
...
1. Legen Sie Laufzeitoptionen fest.
   ...
   * Erstellen Sie ein Objekt `ToImageOptionsSpec`, indem Sie den Konstruktor verwenden.
   * Rufen Sie nach Bedarf Methoden auf, die zu diesem Objekt gehören. Legen Sie beispielsweise den Bildtyp fest, indem Sie die `setImageConvertFormat` Methode aufrufen und einen `ImageConvertFormat` Aufzählungswert übergeben, der den Formattyp angibt.
   >[!NOTE]
   >
   >Das Festlegen des `ImageConvertFormat` Aufzählungswerts ist obligatorisch.
