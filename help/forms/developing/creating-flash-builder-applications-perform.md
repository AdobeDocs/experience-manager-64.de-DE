---
title: Erstellen von Flash-Builder-Anwendungen, die eine SSO-Authentifizierung mithilfe von HTTP-Token durchführen
seo-title: Creating Flash Builder applicationsthat perform SSO authentication using HTTP tokens
description: Erstellen Sie eine Client-Anwendung mit Flash Builder, der eine SSO-Authentication (Single Sign-on) mithilfe von HTTP-Token durchführt. Authentifizieren Sie einen Benutzer einmal für einen Vorgang und verwenden Sie diese Authentication, um mehrere Vorgänge für AEM Forms auszuführen.
...
# Erstellen von Flash Builder-Programmen, die SSO-Authentifizierung mithilfe von HTTP-Token durchführen {#creating-flash-builder-applicationsthat-perform-sso-authentication-using-http-tokens}

Sie können eine Client-Anwendung mit Flash Builder erstellen, der eine SSO-Authentication (Single Sign-on) mithilfe von HTTP-Token durchführt. Angenommen, Sie erstellen eine webbasierte Anwendung mit Flash Builder. Als Nächstes gehen Sie davon aus, dass die Anwendung verschiedene Ansichten enthält, bei denen jede Ansicht einen anderen AEM Forms-Vorgang aufruft. Anstatt einen Benutzer für jeden Forms-Vorgang zu authentifizieren, können Sie eine Anmeldeseite erstellen, über die sich ein Benutzer einmal authentifizieren kann. Nach der Authentication kann ein Benutzer mehrere Vorgänge aufrufen, ohne sich erneut authentifizieren zu müssen. Wenn sich ein Benutzer beispielsweise bei Workspace (oder einer anderen Forms-Anwendung) angemeldet hat, muss er sich nicht erneut authentifizieren.

Obwohl die Client-Anwendung die erforderliche Anwendungslogik für die SSO-Authentication enthält, führt das AEM Forms User Management die tatsächliche Benutzer-Authentication durch. Um einen Benutzer mithilfe von HTTP-Token zu authentifizieren, ruft die Client-Anwendung den Vorgang `authenticateWithHTTPToken` des Authentication-Manager-Service auf. User Management kann Benutzer mithilfe eines HTTP-Tokens authentifizieren. Für nachfolgende Remoting- oder Webservice-Aufrufe an AEM Forms müssen Sie keine Anmeldeinformationen zur Authentication übergeben.
...
Der folgende kurzlebige AEM Forms-Prozess mit dem Namen `MyApplication/EncryptDocument` wird aufgerufen, nachdem ein Benutzer mithilfe von SSO authentifiziert wurde. (Informationen zu diesem Prozess, wie etwa Eingabe- und Ausgabewerte, finden Sie unter [Beispiel für einen kurzlebigen Prozess](/help/forms/developing/aem-forms-processes.md).)

![cf_cf_encryptdocumentprocess2](assets/cf_cf_encryptdocumentprocess2.png)

>[!NOTE]
>
>Dieser Prozess basiert nicht auf einem vorhandenen AEM Forms-Prozess. Um den Code-Beispielen zu folgen, die den Aufruf dieses Prozesses erläutern, erstellen Sie mit Hilfe von Workbench einen Prozess namens `MyApplication/EncryptDocument`. (Siehe [Verwenden von Workbench](https://www.adobe.com/go/learn_aemforms_workbench_63_de).)

Die mit Flash Builder erstellte Client-Anwendung interagiert mit dem Sicherheitsservlet von User Manager, das unter `/um/login` und `/um/logout` konfiguriert wird. Das heißt, die Client-Anwendung sendet während des Starts eine Anfrage an die `/um/login`-URL, um den Status des Benutzers zu ermitteln. Dann antwortet der User Manager mit dem Benutzerstatus. Die Client-Anwendung und das User-Manager-Sicherheitsservlet kommunizieren über HTTP.

**Anfrageformat**

Das Sicherheitsservlet erfordert die folgenden Eingabevariablen:

* `um_no_redirect` - Dieser Wert muss `true` betragen. Diese Variable begleitet alle Anfragen an das User-Manager-Sicherheitsservlet. Außerdem hilft es dem Sicherheitsservlet, die eingehende Anfrage von einem Flex-Client oder anderen Webanwendungen zu unterscheiden.
* `j_username` - Dieser Wert ist der Wert der Anmeldekennung des Benutzers, wie im Anmeldeformular angegeben.
* `j_password` - Dieser Wert ist das entsprechende Kennwort des Benutzers, wie im Anmeldeformular angegeben.

Der `j_password`-Wert ist nur für Berechtigungsanfragen erforderlich. Wenn der Kennwortwert nicht angegeben ist, prüft das Sicherheits-Servlet, ob das von Ihnen verwendete Konto bereits authentifiziert ist. Wenn ja, können Sie fortfahren. Das Sicherheits-Servlet authentifiziert Sie jedoch nicht erneut.

>[!NOTE]
>
>Stellen Sie für eine ordnungsgemäße Verarbeitung von i18n sicher, dass diese Werte im POST-Format vorliegen.

**Antwortformat**

Das unter `/um/login` konfigurierte Sicherheits-Servlet antwortet mithilfe des `URLVariables`-Formats. In diesem Format liegt die Ausgabe des Inhaltstyps als text/plain vor. Die Ausgabe enthält Namenswertpaare, die durch ein kaufmännisches Und-Zeichen (&amp;) getrennt sind. Die Antwort enthält die folgenden Variablen:

* `authenticated` - Der Wert lautet `true` oder `false`.
* `authstate` - Dieser Wert kann einen der folgenden Werte enthalten:

   * `CREDENTIAL_CHALLENGE` - Dieser Status gibt an, dass der User Manager die Identität des Benutzers auf keine Weise ermitteln kann. Damit die Authentifizierung erfolgt, müssen Benutzername und Kennwort des Benutzers angegeben werden.
   * `SPNEGO_CHALLENGE`- Dieser Status wird wie `CREDENTIAL_CHALLENGE` behandelt.
   * `COMPLETE` - Dieser Status gibt an, dass der User Manager den Benutzer authentifizieren kann.
   * `FAILED` - Dieser Status zeigt an, dass der User Manager den Benutzer nicht authentifizieren konnte. Als Antwort auf diesen Status kann der Flex-Client dem Benutzer eine Fehlermeldung anzeigen.
   * `LOGGED_OUT` - Dieser Status zeigt an, dass sich der Benutzer erfolgreich abgemeldet hat.

* `assertionid` - Wenn der Status `COMPLETE` war, dann enthält er den `assertionId`-Wert des Benutzers. Eine Client-Anwendung kann das `AuthResult` für den Benutzer erhalten.

**Anmeldeprozess**

Wenn eine Client-Anwendung gestartet wird, können Sie eine POST-Anfrage an das `/um/login`-Sicherheits-Servlet stellen. Beispiel: `https://<your_serverhost>:<your_port>/um/login?um_no_redirect=true`. Wenn die Anfrage das User Manager-Sicherheits-Servlet erreicht, führt sie die folgenden Schritte aus:
...
1. Wenn Header-basierte SSO aktiviert ist, sucht das Servlet nach konfigurierten Kopfzeilen, um die Identität des Benutzers zu ermitteln.
...
Wenn das Sicherheits-Servlet ein gültiges Token findet, das einem Benutzer entspricht, lässt das Sicherheits-Servlet Sie fortfahren und antwortet mit `authstate=COMPLETE`. Andernfalls antwortet das Sicherheits-Servlet mit `authstate=CREDENTIAL_CHALLENGE`. In der folgenden Liste werden diese Werte erläutert:

* `Case authstate=COMPLETE`: Gibt an, dass der Benutzer authentifiziert ist und der `assertionid`-Wert die Bestätigungskennung für den Benutzer enthält. In dieser Phase kann die Client-Anwendung eine Verbindung zu AEM Forms herstellen. Das für diese URL konfigurierte Servlet kann das `AuthResult` für den Benutzer durch Aufrufen der `AuthenticationManager.authenticate(HttpRequestToken)`-Methode erhalten. Die `AuthResult`-Instanz kann den Kontext des Benutzermanagers erstellen und in der Sitzung speichern.
* `Case authstate=CREDENTIAL_CHALLENGE`: Gibt an, dass das Sicherheits-Servlet die Anmeldeinformationen des Benutzers benötigt. Als Antwort kann die Client-Anwendung dem Benutzer den Anmeldebildschirm anzeigen und die erhaltenen Anmeldedaten an das Sicherheits-Servlet senden (beispielsweise `https://<your_serverhost>:<your_port>/um/login?um_no_redirect=true&j_username=administrator&j_password=password)`. Wenn die Authentifizierung erfolgreich ist, antwortet das Sicherheits-Servlet mit `authstate=COMPLETE`.

Wenn die Authentifizierung immer noch nicht erfolgreich ist, antwortet das Sicherheits-Servlet mit `authstate=FAILED`. Um auf diesen Wert zu reagieren, kann die Client-Anwendung eine Nachricht anzeigen, um die Anmeldeinformationen erneut abzurufen.

>[!NOTE]
>
>Während `authstate=CREDENTIAL_CHALLENGE`, wird empfohlen, dass der Client die erhaltenen Anmeldeinformationen in einem POST-Formular an das Sicherheits-Servlet sendet.
...
Nach Erhalt dieser Anfrage löscht das User Manager-Sicherheits-Servlet das `lcAuthToken`Cookie und antwortet mit `authstate=LOGGED_OUT`. Nachdem die Client-Anwendung diesen Wert erhalten hat, kann die Anwendung Bereinigungsaufgaben durchführen.

## Erstellen einer Client-Anwendung, die AEM Forms-Benutzer mit SSO authentifiziert {#creating-a-client-application-that-authenticates-aem-forms-users-using-sso}

Um zu demonstrieren, wie Sie eine Client-Anwendung erstellen, die eine SSO-Authentifizierung durchführt, wird eine Beispiel-Client-Anwendung erstellt. Die folgende Abbildung zeigt die Schritte, welche die Client-Anwendung zur Authentifizierung eines Benutzers mit SSO durchführt.

![cf_cf_flexsso](assets/cf_cf_flexsso.png)

Die vorherige Abbildung beschreibt den Anwendungsfluss, der beim Starten der Client-Anwendung auftritt.

1. Die Client-Anwendung löst das `applicationComplete`-Ereignis aus.
1. Der Aufruf von `ISSOManager.singleSignOn` erfolgt. Die Client-Anwendung sendet eine Anfrage an das User Manager-Sicherheits-Servlet.
1. Wenn das Sicherheits-Servlet den Benutzer authentifiziert, dann sendet `ISSOManager` `SSOEvent.AUTHENTICATION_SUCCESS`. Als Antwort zeigt die Client-Anwendung die Hauptseite an. In diesem Beispiel ruft die Hauptseite den kurzlebigen AEM Forms-Prozess mit dem Namen „MyApplication/EncryptDocument“ auf.
1. Wenn das Sicherheits-Servlet nicht ermitteln kann, ob der Benutzer gültig ist, fordert die Anwendung erneut Benutzeranmeldeinformationen an. Die `ISSOManager`-Klasse löst das `SSOEvent.AUTHENTICATION_REQUIRED`-Ereignis aus. Die Client-Anwendung zeigt die Anmeldeseite an.
1. Die auf der Anmeldeseite angegebenen Anmeldeinformationen werden an die `ISSOManager.login`-Methode gesendet. Wenn die Authentifizierung erfolgreich ist, führt sie zu Schritt 3. Andernfalls wird das `SSOEvent.AUTHENTICATION_FAILED`-Ereignis ausgelöst. Die Clientanwendung zeigt die Anmeldeseite und eine entsprechende Fehlermeldung an.

### Client-Anwendung erstellen {#creating-the-client-application}

Die Client-Anwendung besteht aus den folgenden Dateien:

* `SSOStandalone.mxml`: Die MXML-Hauptdatei, welche die Client-Anwendung darstellt. (Siehe [Erstellen der Datei „SSOStandalone.mxml“](creating-flash-builder-applications-perform.md#creating-the-ssostandalone-mxml-file).)
...
* `um/SSOEvent.as`: Das `SSOEvent` wird für SSO-bezogene Ereignisse gesendet. (Siehe [Erstellen der Datei „SSOEvent.as“](creating-flash-builder-applications-perform.md#creating-the-ssoevent-as-file).)
...
* `um/UserManager.as`: Enthält die Anwendungslogik, die den Authentication-Manager-Service mithilfe seiner WSDL aufruft. (Siehe [Erstellen der Datei „UserManager.as“](creating-flash-builder-applications-perform.md#creating-the-usermanager-as-file).)
...
Die folgende Abbildung zeigt eine visuelle Darstellung der Client-Anwendung.

![cf_cf_sso_project](assets/cf_cf_sso_project.png)

>[!NOTE]
>
>Beachten Sie, dass es zwei Pakete mit den Namen um und views gibt. Stellen Sie beim Erstellen der Client-Anwendung sicher, dass Sie die Dateien in den entsprechenden Paketen platzieren. Stellen Sie außerdem sicher, dass Sie die Datei „adobe-remoting-provider.swc“ zum Klassenpfad Ihres Projekts hinzufügen. (Siehe [Einschließen der Flex-Bibliotheksdatei für AEM Forms](/help/forms/developing/invoking-aem-forms-using-remoting.md#including-the-aem-forms-flex-library-file).)
...
Die folgenden Abschnitte enthalten zusätzliche Details, die die Kommunikation zwischen der Client-Anwendung und dem Sicherheits-Servlet des User Managers beschreiben.
...
Das Sicherheits-Servlet des User Managers antwortet mit dem folgenden Wert:

`HTTP/1.1 200 OK`

`authenticated=false&authstate=CREDENTIAL_CHALLENGE`

Als Antwort auf diesen Wert wird ein `SSOEvent.AUTHENTICATION_REQUIRED`-Wert gesendet. Daher zeigt die Client-Anwendung dem Benutzer einen Anmeldebildschirm an. Die Anmeldeinformationen werden zurück an das Sicherheits-Servlet des User Managers gesendet.
...
Daher wird `authstate=COMPLETE the SSOEvent.AUTHENTICATION_SUCCESS` gesendet. Die Client-Anwendung kann bei Bedarf weitere Verarbeitungsschritte durchführen. Beispielsweise kann ein Protokoll erstellt werden, welches das Datum und die Uhrzeit der Authentifizierung des Benutzers erfasst.

### Der Benutzer ist bereits authentifiziert {#the-user-is-already-authenticated}

In diesem Fall hat sich der Benutzer bereits bei AEM Forms angemeldet und navigiert dann zur Client-Anwendung. Die Client-Anwendung stellt während des Starts eine Verbindung zum User Manager-Sicherheits-Servlet her.
...
Da der Benutzer bereits authentifiziert ist, ist das User Manager-Cookie vorhanden und wird an das User Manager-Sicherheits-Servlet gesendet. Das Servlet ruft dann den `assertionId`-Wert ab und überprüft, ob er gültig ist. Wenn er gültig ist, wird `authstate=COMPLETE` zurückgegeben. Sonst wird `authstate=CREDENTIAL_CHALLENGE` zurückgegeben. Im Folgenden finden Sie eine typische Antwort:
...
In diesem Fall wird dem Benutzer kein Anmeldebildschirm angezeigt und er wird stattdessen direkt zu einem Willkommensbildschirm geleitet.
