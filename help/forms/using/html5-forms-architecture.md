...
Die HTML5 Forms-Funktionalität wird als Paket innerhalb der eingebetteten AEM-Instanz bereitgestellt und mithilfe von RESTful als REST-Endpunkt über HTTP/S bereitgestellt [Apache Sling-Architektur](https://sling.apache.org/).
...
Weitere Informationen zu REST-Endpunkten und unterstützten Anforderungsparametern finden Sie unter [Rendern von Formularvorlagen](/help/forms/using/rendering-form-template.md).
...
![HTML5-Formular-Workflow](assets/cacheworkflow.png)
...
Wenn ein Benutzer das Formular sendet, sendet der Browser den Formularstatus im JSON-Format an die [Sendedienst-Proxy](/help/forms/using/service-proxy.md); dann generiert der Sendedienst-Proxy eine Daten-XML mithilfe von JSON-Daten und sendet diese Daten-XML an den Senden-Endpunkt.
...
Wenn eine Renderanfrage empfangen wird, ruft diese Komponente den Forms-Dienstcontainer auf, um Layout- und Statusinformationen zu generieren, die weiter verarbeitet werden, um HTML- und JSON-Formular-DOM-Status zu generieren.
...
Der Profilknoten hat ein Eigenschafts-**Sling: resourceSuperType** mit dem Wert **xfaforms/profile**. Diese Eigenschaft sendet intern Anforderungen an das Sling-Skript für Profilknoten, die sich im **/libs/xfaforms/profile** Ordner. Diese Skripten sind JSP-Seiten, die als Container für die Zusammenfügung der HTML-Formulare und der erforderlichen JS/CSS-Artefakte dienen. Die Seiten enthalten Verweise auf:
