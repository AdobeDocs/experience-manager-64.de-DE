...
*In dem diskutierten Beispiel sind oben in der benutzerdefinierten Vorlage zwei Div-Elemente vorhanden. Das erste mit der CSS-Klasse „__FP_boxes-container“ fungiert als Containerelement für die aufgelisteten Formulare. Das zweite mit der CSS-Klasse „__FP_boxes“ ist eine Vorlage für die Basiseinheiten, in diesem Fall ein Formular. The **data-repeatable** attribute present in the Div element has the value **true**.

Jeder Platzhalter verfügt über einen exklusiven OOTB-Metadatensatz. To display custom metadata at a particular place on the form, add the **$metadata_prop property** at the place.

*Im Beispiel wird die Metadaten-Eigenschaft in mehreren Instanzen verwendet. For example, it is used in **description**,**name**,**formUrl**,**htmlStyle**,**pdfUrl**,**pdfStyle**, and **path**in the prescribed manner.*
...
* **assetType**: Typ des Assets Valid values include **Form**, **PDF Form**, **Print Form**, and **Adaptive Form**
* **htmlStyle**&amp; **pdfStyle**: Anzeigestil für HTML- bzw. PDF-Symbole für die Ausgabe. Valid values are “**__FP_display_none**” or **blank**
...
1. **Verwenden von Konfigurationseigenschaften**: Die Komponente „Search &amp; Lister“ verfügt über mehrere Konfigurationen, die Sie in der Benutzeroberfläche verwenden können. For example, to display HTML ToolTip text saved through the edit dialog, use the **${config-htmlLinkText} attribute.** Verwenden Sie für PDF-QuickInfo-Text das **Attribut ${config-pdfLinkText}** .
...
1. Geben Sie der CSS-Klasse für die Löschoption im Bereich „Drafts“ in der Komponente „Drafts &amp; Submissions“ den Namen „__FP_deleteDraft“. In addition, include the attribute &quot;draftID&quot; with the value **${draftID}**, which is the draft id of corresponding draft.
