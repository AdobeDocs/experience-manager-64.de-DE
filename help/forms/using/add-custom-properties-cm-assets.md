...
1. Unter dem neu erstellten Überlagerungsknoten (/apps/fd/cm/ma/gui/content/cmmetadataproperties/commonproperties/col1/items)
   ...
   Erstellen Sie für jede Eigenschaft (Felder) einen Knoten, für den Sie eine Dropdown-Liste erstellen müssen (hier) `geographicallocation`) des Typs nt:unstructured.
...
1. Fügen Sie unter dem property-Knoten (hier geographicallocation) einen neuen Knoten mit dem Namen `items` hinzu. Fügen Sie unter dem Knoten items jeweils einen Knoten für die Werte in der Dropdown-Liste hinzu. Als Best Practice empfiehlt es sich, den ersten Knoten als leer hinzuzufügen, um als Standardwert der Dropdown-Liste und eine Option zu verwenden, mit der der Benutzer keinen Wert für das Feld angeben kann. Um mehrere Optionen/Dropdown-Werte hinzuzufügen, wiederholen Sie die folgenden Schritte:
   ...
   1. Fügen Sie die folgenden Eigenschaften dem neu erstellten Knoten (hier: item1) hinzu und klicken Sie auf **Alle speichern**:
      ...
      | Name | Typ | Wert |
      |--- |--- |--- |
      | text | Zeichenfolge | Dies ist der Wert der Dropdown-Option, die für den Benutzer sichtbar ist. Leer lassen für den leeren (Standardwert) Wert oder geben Sie den Wert ein, z. B. **International** oder **Innerhalb der USA**. |
...
1. Fügen Sie unter dem neu erstellten Ordner „items“ einen Knoten für die benutzerdefinierte Registerkarte im Asset hinzu (Beispiel: customtab), indem Sie folgende Schritte durchführen:
   ...
   1. Klicken Sie auf den neu erstellten Knoten (hier customtab). CRX zeigt die Eigenschaften des Knotens an.
   1. Fügen Sie die folgenden zwei Eigenschaften zum Knoten hinzu (hier customtab):
...
1. Erstellen Sie für jede der erstellten Eigenschaften einen Knoten unter dem Spaltenknoten, um eine Spalte in der Benutzeroberfläche zu erstellen. Wiederholen Sie diesen Schritt, um weitere Spalten in der Benutzeroberfläche zu erstellen:
   ...
   1. Fügen Sie dem Knoten die folgenden Eigenschaften hinzu und klicken Sie auf **Alle speichern**:
      ...
      | sortable | Boolesch | Ja Der Wert true bedeutet, dass der Benutzer die Werte in dieser Spalte sortieren kann. |
...
1. Öffnen Sie die Datei &quot;childlistpage.jsp&quot;(/apps/fd/cm/ma/gui/components/admin/childpagerenderer/childlistpage/childlistpage.jsp) und nehmen Sie die folgenden Änderungen vor:
   ...
   1. Fügen Sie am Ende der Datei den folgenden Code einer Funktion hinzu, die Werte für jede benutzerdefinierte Eigenschaft abruft:
...
1. (Optional) Standardmäßig wird die neue Spalte als letzte Spalte in der Benutzeroberfläche angezeigt. Damit die Spalte an einer bestimmten Position angezeigt wird, fügen Sie dem Spaltenknoten die folgende Eigenschaft hinzu:
   ...
   | Name | Typ | Wert |
   |--- |--- |--- |
   | sling:orderBefore | Zeichenfolge | Der Name des Spaltenknotens im Pfad &quot;`/libs/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns`&quot;, vor der die benutzerdefinierte Spalte in der Benutzeroberfläche angezeigt werden muss. Wenn die Spalte &quot;Geografischer Standort&quot;vor der Spalte &quot;Version&quot;angezeigt werden soll, fügen Sie hier die Eigenschaft hinzu `sling:orderBefore` zum GeoLocation-Knoten unter Pfad &quot;`/apps/fd/cm/ma/gui/content/cmassets/jcr:content/views/list/columns/GeoLocation`und legen Sie den Wert der Eigenschaft auf Version fest. |
