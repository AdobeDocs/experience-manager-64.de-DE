...
* Adaptive Formulare bieten Bereichslayouts, z. B. Responsive Layouts, Bedienfelder mit Registerkarten, Akkordeons und Assistenten, um zu steuern, wie Formularkomponenten in einem Bereich angeordnet werden. Sie können benutzerdefinierte Bedienfeldlayouts erstellen und für Formularverfasser verfügbar machen. For more information, see [Creating custom layout components for adaptive forms](/help/forms/using/custom-layout-components-forms.md).
...
* To view and edit the properties of a component in the components browser in the sidebar, select the component and click ![cmppr-1](assets/cmppr-1.png). Sie können auch auf eine Komponente doppelkicken, um ihre Eigenschaften im Eigenschaftenbrowser anzuzeigen.
* Verwenden Sie die Tastaturkürzel, um schnelle Aktionen in Ihren Formularen durchzuführen. See [AEM Forms Keyboard Shortcuts](/help/forms/using/keyboard-shortcuts.md).
...
* Aktivieren Sie HTML-Client-Bibliothekskomprimierung für JavaScript und CSS-Code aus der Felix-Konsole. See [Clientlibs explained by example](https://blogs.adobe.com/experiencedelivers/experience-management/clientlibs-explained-example/).
* Cache all client libraries at `/etc.clientlibs/fd` and any additional custom client libraries on AEM dispatcher to increase the responsiveness and security of your published forms. Weitere Informationen finden Sie unter [Dispatcher](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher.html)

* Nicht zwischenspeichern `/content/forms/af/` und `/content/dam/formsanddocuments/*` Pfade. Weitere Informationen zum Konfigurieren der Zwischenspeicherung von adaptiven Formularen finden Sie unter [Zwischenspeichern von adaptiven Formularen](/help/forms/using/configure-adaptive-forms-cache.md).
...
* Sichere AEM Forms-Server.
   ...
   * Verwenden Sie `nosamplecontent`-Laufzeitmodus unter, um sicherzustellen, dass die Anwendung keine Beispielinhalte und Beispielbenutzer enthält, die auf dem Produktionsserver bereitgestellt werden. See [Running AEM in Production Ready Mode](/help/sites-administering/production-ready.md).
...
* Verwenden Sie einen sicheren externen Speicherort wie eine Datenbank, um Daten aus dem Entwurf und dem übermittelten Formular zu speichern. See [Configuring external storage for drafts and submitted forms data](/help/forms/using/adaptive-forms-best-practices.md#external-storage).
