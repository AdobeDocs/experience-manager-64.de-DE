...
* [Best Practices: Bereitstellen und Verwalten von AEM](/help/sites-deploying/best-practices.md)
...
* Mit dem FileVault-Werkzeug oder VLT können Sie den Inhalt einer CRX- oder AEM-Instanz auf Ihr Dateisystem zuordnen. Es bietet Änderungskontrollmanagementvorgänge, wie z. B. das Einchecken und Auschecken des AEM-Projektinhalts. Siehe [Verwendung des VLT-Tools](/help/sites-developing/ht-vlttool.md).
...
* Adaptive Formulare bieten Bereichslayouts, z. B. Responsive Layouts, Bedienfelder mit Registerkarten, Akkordeons und Assistenten, um zu steuern, wie Formularkomponenten in einem Bereich angeordnet werden. Sie können benutzerdefinierte Bedienfeldlayouts erstellen und für Formularverfasser verfügbar machen. Weitere Informationen finden Sie unter [Erstellen benutzerdefinierter Layoutkomponenten für adaptive Formulare](/help/forms/using/custom-layout-components-forms.md).
...
* Für adaptive Formulare, die auf dem Formulardatenmodell basieren, stellt AEM Forms den vordefinierten Vorbefüllungs-Dienst für Formulardatenmodelle bereit. Der Prefill-Dienst fragt nach Datenquellen für Datenmodellobjekte im adaptiven Formular und befüllt Feldwerte beim Rendern des Formulars.
...
Weitere Informationen finden Sie unter [Vorausfüllen von Feldern in adaptiven Formularen](/help/forms/using/prepopulate-adaptive-form-fields.md).
...
* Adobe Sign-aktiviertes Formular wird nur gesendet, nachdem alle Unterzeichner das Formular unterzeichnet haben. Forms wird im Status &quot;Ausstehende Signatur&quot;angezeigt, bis das Formular von allen Unterzeichnern unterzeichnet wurde.
...
Weitere Informationen finden Sie unter [AEM Chrome-Plug-in - Adaptives Formular](https://adobe-consulting-services.github.io/acs-aem-tools/aem-chrome-plugin/adaptive-form/).

Calvin SDK ist eine Dienstprogramm-API für Entwickler von adaptiven Formularen zum Testen von adaptiven Formularen. Calvin SDK basiert auf dem [Hobbes.js-Test-Framework](https://docs.adobe.com/docs/de/aem/6-3/develop/ref/test-api/index.html). Sie können den Rahmen verwenden, um Folgendes zu testen:
...
* Alle Modifizierungen wie das Hinzufügen einer neuen Komponente oder das Anwenden eines Skripts in einem lokalisierten Formular, werden nicht automatisch lokalisiert. Daher müssen Sie ein Formular vor der Lokalisierung abschließen, um mehrere Lokalisierungszyklen zu vermeiden.
* Verwenden Sie den Anforderungsparameter `afAcceptLang` , , um das Browsergebietsschemazu überschreiben und das Formular in einem spezifischen Gebietsschema zu lokalisieren. Beispielsweise erzwingt die folgende URL die Wiedergabe des Formulars in japanischem Gebietsschema, unabhängig vom in der Browsereinstellung angegebenen Gebietsschema:
...
* Sichere AEM Forms-Server.
   ...
   * Verwenden Sie `nosamplecontent`-Laufzeitmodus unter, um sicherzustellen, dass die Anwendung keine Beispielinhalte und Beispielbenutzer enthält, die auf dem Produktionsserver bereitgestellt werden. Siehe [Ausführen von AEM im produktionsbereiten Modus](/help/sites-administering/production-ready.md).
