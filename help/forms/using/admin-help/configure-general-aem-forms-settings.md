...
1. Ändern Sie auf der Seite &quot;Core-Konfigurationen&quot;die Optionen nach Bedarf und klicken Sie auf **[!UICONTROL OK]**. Einzelheiten zu den Optionen finden Sie unter [Optionen für die Core-Konfigurationen](configure-general-aem-forms-settings.md#core-configurations-options).

## Optionen für die Core-Konfigurationen {#core-configurations-options}

**Speicherort des temporären Ordners** Der Ordnerpfad, in dem AEM Formulare temporäre Produktdateien erstellen. Wenn der Wert dieser Einstellung leer ist, wird als Speicherort standardmäßig ein Ordner unterhalb des Systemordners „temp“ gewählt. Stellen Sie sicher, dass der temporäre Ordner ein beschreibbarer Ordner ist. 
...
**Speicherort des Ordners für Systemschriftarten** Geben Sie den Pfad zum Schriftartenordner ein, den Ihr Betriebssystem bereitgestellt hat. Es können mehrere Ordner hinzugefügt werden, die durch ein Semikolon getrennt sind **;**.

**Speicherort der Konfigurationsdatei für Data Services** Gibt den Speicherort der Datei services-config.xml an. Standardmäßig ist diese Datei in die Datei &quot;adobe-core-appserver.ear&quot;eingebettet und kann nicht vom Benutzer aufgerufen werden. Eine Kopie der standardmäßigen Datei services-config.xml befindet sich unter [AEM-Forms-Stamm]\sdk\misc\DataServices\Server-Configuration Wenn Sie Änderungen an dieser Datei vorgenommen und sie verschoben haben, geben Sie in dieses Feld den neuen Speicherort ein.
...
**WSDL aktivieren** Wählen Sie diese Option, um die WSDL-Generierung (Web Service Definition Language) für alle Dienste zu aktivieren, die Teil AEM Formulare sind.
