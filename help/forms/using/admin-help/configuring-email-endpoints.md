...
**Dateimuster:** Die Muster für eingehende Dateianhänge, die vom Anbieter akzeptiert werden. Dazu gehören Dateien mit bestimmten Erweiterungen (&amp;ast;.dat, &amp;ast;.xml), bestimmten Ausdrücken (Daten) und Composite-Dateien mit Namen und Erweiterung (.[dD][aA][Tt]). Der Standardwert ist &amp;ast;.&amp;ast;.
...
**Dateimuster:** Gibt die Muster für eingehende Dateianhänge an, die vom Anbieter akzeptiert werden. Dazu gehören Dateien mit bestimmten Erweiterungen (&amp;ast;.dat, &amp;ast;.xml), bestimmten Ausdrücken (Daten) oder zusammengesetzten Namen in Name und Erweiterung (&amp;ast;..[dD][aA][Tt]).
