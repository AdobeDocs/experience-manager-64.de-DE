...
AEM Forms ist eine leistungsstarke Plattform der Enterprise-Klasse. Forms-zentrierter Arbeitsablauf auf OSGi ist nur eine der Funktionen von AEM Forms. Eine vollständige Liste der Funktionen finden Sie unter [Einführung in AEM Forms](/help/forms/using/introduction-aem-forms.md).

>[!NOTE]
>
>Mit formularzentrierten Workflows in OSGi können Sie schnell Workflows für verschiedene Aufgaben auf dem OSGi-Stapel erstellen und bereitstellen, ohne die komplette Prozessverwaltungsfunktion auf dem JEE-Stapel zu installieren. Sehen Sie sich einen [Vergleich](/help/forms/using/capabilities-osgi-jee-workflows.md) der Forms-zentrierten AEM Workflows zu OSGi und Process Management on JEE an, um mehr über die Unterschiede und Ähnlichkeiten in den Funktionen zu erfahren.
>
>Nach dem Vergleich finden Sie unter Installieren oder Aktualisieren von AEM Forms on JEE [](/help/forms/home.md) ausführliche Informationen zum Installieren und Konfigurieren des JEE-Stapels und zu den Process Management-Funktionen, wenn Sie die Process Management-Funktion auf JEE-Stapel installieren.

## Bereitstellungstopologie {#deployment-topology}

AEM Forms-Add-On-Paket ist eine Anwendung, die auf AEM bereitgestellt wird. Sie benötigen nur mindestens eine AEM Author- oder Verarbeitungsinstanz (Produktionsautor), um den Forms-zentrierten Workflow mit OSGi-Funktionen auszuführen. A processing instance is a [hardened AEM Author](/help/forms/using/hardening-securing-aem-forms-environment.md) instance. Führen Sie kein tatsächliches Authoring wie das Erstellen von Workflows oder adaptiven Formularen im Produktionsautor durch.
...
>[!NOTE]
>
>Gehen Sie zum Abschnitt [Nächste Schritte](#next-steps) des Dokuments, wenn Sie AEM Forms bereits auf OSGi installiert haben, wie im Artikel [Installations- und Konfigurationsfunktionen](/help/forms/using/installing-configuring-aem-forms-osgi.md) beschrieben.
...
* [Forms-zentrierter Workflow unter OSGi](/help/forms/using/aem-forms-workflow.md)
