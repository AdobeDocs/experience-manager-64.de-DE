...
* Trennung zwischen der Einrichtung der Zugriffssteuerung für den eingeschränkten Lesezugriff, die normalerweise in Autoreninstanzen benötigt wird, und der Berechtigungsüberprüfung, die normalerweise nur bei der Veröffentlichung gewünscht wird;
...
* Zugriffssteuerungsrichtlinie des Typs org.apache.jackrabbit.api.security.approval.PrincipalSetPolicy (durch die Apache Jackrabbit-API definiert);
* PrincipalSetPolicy gewährt Privilegien für einen veränderlichen Satz von Prinzipalen.
...
Diese CUG-Richtlinien werden über ein separates Autorisierungsmodul namens oak-approval-cug auf einer AEM-Instanz bereitgestellt. Dieses Modul verfügt über eine eigene Zugriffssteuerungsverwaltung und Berechtigungsprüfung. Das heißt, die Standardversion von AEM umfasst eine Oak-Content-Repository-Konfiguration, die mehrere Autorisierungsmechanismen kombiniert. For more info, see [this page on the Apache Oak Documentation](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html).

In diesem Composite-Setup ersetzt ein neues CUG nicht den vorhandenen Zugriffssteuerungsinhalt, der an den Zielknoten angehängt ist, sondern ist als Ergänzung konzipiert, die später auch entfernt werden kann, ohne dass die ursprüngliche Zugriffssteuerung beeinträchtigt wird, die standardmäßig in AEM eine Zugriffssteuerungsliste wäre.

Anders als bei der vorherigen Implementierung werden neue CUG-Richtlinien immer als Zugriffssteuerungsinhalt erkannt und behandelt. Dies bedeutet, dass sie mit der JCR-Zugriffssteuerungsverwaltungs-API erstellt und bearbeitet werden. For more info, see the [Managing CUG Policies](#managing-cug-policies) section.
...
* Es werden weder Schreibberechtigungen noch Berechtigungen verarbeitet, die für die Änderung von geschützten JCR-Inhalten erforderlich sind (Zugriffssteuerung, Informationen zum Node-Typ, Versionierung, Sperren oder Benutzerverwaltung unter anderem); Diese Berechtigungen sind nicht von einer CUG-Richtlinie betroffen und werden nicht vom zugehörigen Autorisierungsmodell bewertet. Ob diese Berechtigungen gewährt werden, hängt von den anderen Modellen ab, die in der Sicherheitseinrichtung konfiguriert sind.
...
* Beschränken Sie die Pfade mit Unterstützung für CUG-Richtlinien auf einige wenige Baumstrukturen im Repository, um eine optimale Leistung zu ermöglichen. Beispielsweise können Sie CUGs unter dem Knoten /content nur als Standardwert seit AEM 6.3 zulassen.
...
Die Teile des CUG-Features, die für Authentifizierung relevant sind, ermöglichen die Markierung von Baumstrukturen, die Authentifizierung erfordern, und optional die Angabe einer dedizierten Anmeldeseite. In accordance to the previous version, the new implementation allows to mark trees that require authentication in the content repository and conditionally enable synchronization with the `Sling org.apache.sling.api.auth.Authenticator`responsible for ultimately enforcing the requirement and redirecting to a login resource.
...
Das standardmäßige AEM-Setup nutzt diese Konfiguration jetzt, indem es ermöglicht, das mixin im Autorenmodus festzulegen, jedoch erst bei der Replikation auf die Veröffentlichungsinstanz wirksam zu machen. See [this page](https://sling.apache.org/documentation/the-sling-engine/authentication/authenticationframework.html) for details how Sling enforces the authentication requirement.
...
Upon calling `AuthenticationHandler.requestCredentials` this handler makes an attempt to determine the mapping login page to which the user will be redirected. Dies umfasst die folgenden Schritte:
...
The target of this documentation is the evaluation of the login path as exposed by the internal `LoginPathProvider` interface. Die seit Einführung von AEM 6.3 enthaltene Implementierung verhält sich wie folgt:
...
* Zur Vermeidung genauer Angaben und der darauf folgenden Registrierung redundanter Anmeldepfade:
   ...
   * Nutzen Sie die Vererbung und vermeiden Sie die Erstellung verschachtelter Anmeldepfade.
...
Die Notwendigkeit einer separaten Authentifizierung wird im Repository-Inhalt widergespiegelt, wobei ein dedizierter Mixin-Knotentyp auf dem Zielknoten platziert wird. Der Mixin-Typ definiert eine optionale Eigenschaft, um für die vom Zielknoten definierte Baumstruktur eine dedizierte Anmeldeseite festzulegen.
...
Code für die Anwendung einer neuen CUG-Richtlinie auf einen Knoten, der vorher keinen CUG-Satz aufwies. Please note that `getApplicablePolicies` will only return new policies that have not been set before. Am Ende muss die Richtlinie zurückgeschrieben und Änderungen müssen beibehalten werden.
...
Die JCR-Zugriffssteuerungsverwaltung definiert eine optimale Methode zum Abrufen der Richtlinien, die an einem gegebenen Pfad wirksam sind. Due to the fact that evaluation of CUG policies is conditional and depends on the corresponding configuration to be enabled, calling `getEffectivePolicies` is a convenient way to verify if a given CUG policy is taking effect in a given installation.
...
Im Folgenden finden Sie Schritte zum Erstellen einer neuen Authentifizierungspflicht. Note that the requirement will only be registered with the Apache Sling Authenticator if the `RequirementHandler` has been configured for the tree containing the target node.
...
Im Folgenden finden Sie Schritte zum Ändern eines vorhandenen Anmeldepfads. The modification will only be registered with the Apache Sling Authenticator if the `RequirementHandler` has been configured for the tree containing the target node. Der vorherige Anmeldepfadwert wird aus der Registrierung entfernt. Die mit dem Zielknoten verknüpfte Authentifizierungspflicht ist von dieser Änderung nicht betroffen.
...
Wie beim Anmeldepfad gibt es keine öffentlichen APIs zum Abrufen der im Inhalt definierten geerbten Authentifizierungspflichten. Im folgenden Beispiel wird veranschaulicht, wie alle Authentifizierungsanforderungen, die mit einer bestimmten Hierarchie definiert wurden, unabhängig davon, ob sie wirksam werden oder nicht, aufgelistet werden. Weitere Informationen finden Sie unter [Konfigurationsoptionen](/help/sites-administering/closed-user-groups.md#configuration-options).
...
|---|---|---|---|
| Ja | Ja | Ja | Ein bestimmter Benutzer kann die mit der CUG-Richtlinie markierte Unterstruktur nur anzeigen, wenn eine effektive Berechtigungsbewertung den Zugriff erlaubt. Ein nicht authentifizierter Benutzer wird an die angegebene Anmeldeseite umgeleitet. |
| Ja | Nein | Ja | Ein bestimmter Benutzer kann die mit der CUG-Richtlinie markierte Unterstruktur nur anzeigen, wenn eine effektive Berechtigungsbewertung den Zugriff erlaubt. Ein nicht authentifizierter Benutzer wird an eine übernommene Standardanmeldeseite umgeleitet. |
...
Setting up CUG authorization is described in detail in the [relevant Apache Documentation](https://jackrabbit.apache.org/oak/docs/security/authorization/cug.html#pluggability). Standardmäßig stellt AEM die CUG-Autorisierung in allen Ausführungsmodi bereit. Die schrittweisen Anweisungen können auch verwendet werden, um die CUG-Autorisierung in Installationen zu deaktivieren, die eine andere Autorisierungseinrichtung erfordern.
...
Alternativ können Sie eine benutzerdefinierte Implementierung der CugExclude-Schnittstelle bereitstellen und bereitstellen, um die Gruppe ausgeschlossener Prinzipale bei besonderen Bedürfnissen anzupassen. See the documentation on [CUG pluggability](https://jackrabbit.apache.org/oak/docs/security/authorization/cug.html#pluggability) for details and an example implementation.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>Beschreibung</td> 
   <td>Spezifischer OSGi-Dienst für Authentifizierungsanforderungen, der einen Beobachter für Inhaltsänderungen registriert, die sich auf die Authentifizierungsanforderungen auswirken (durch den <code>granite:AuthenticationRequirement</code> Mixin-Typ), und Anmeldepfade, mit denen der <code>LoginSelectorHandler</code>... </td> 
  </tr> 
  <tr> 
   <td>Konfigurationseigenschaften</td> 
   <td>-</td> 
...
| Etikett | Adobe Granite-Authentifizierungsanforderungen und Anmeldungspfad-Handler |
|---|---|
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Set&lt;String&gt;</td> 
   <td>-</td> 
   <td>Pfade, unter denen Authentifizierungsanforderungen von diesem Handler eingehalten werden. Leave this configuration unset if you want to add the <code>granite:AuthenticationRequirement</code> mixin type to nodes without having them enforced (for example, on author instances). Wenn nicht vorhanden, ist die Funktion deaktiviert. </td> 
  </tr> 
 </tbody> 
</table>
...
|---|---|
| Unterstützte Pfade `/content` | Zugriffssteuerungsverwaltung für CUGpolicies ist aktiviert. |
...
| **&quot;Apache Jackrabbit Oak CUG Configuration&quot;** | **Erklärung** |
|---|---|
| Unterstützte Pfade `/content` | Die Verwaltung der Zugriffssteuerung für CUG-Richtlinien ist unterhalb der konfigurierten Pfade aktiviert. |
| CUG-Bewertung aktiviert TRUE | Die Berechtigungsauswertung ist unterhalb der konfigurierten Pfade aktiviert. Die CUG-Politik wird mit Wirkung `Session.save()`. |
| Rangansicht | 200 | Siehe Oak-Dokumentation. |

| **&quot;Apache Jackrabbit Oak CUG Exclude List&quot;** | **Erklärung** |
|---|---|
| Prinzipalnamenadministratoren | Schließt Administratorprinzipal aus der CUG-Auswertung aus. |

| **&quot;Adobe Granite Authentication Requirement and Login Path Handler&quot;** | **Erklärung** |
|---|---|
| Unterstützte Pfade `/content` | Die Authentifizierungsanforderungen, wie sie im Repository mithilfe des `granite:AuthenticationRequired` Mixin-Typs definiert sind, treten `/content` bei `Session.save()`ein. Sling Authenticator wird aktualisiert. Wird der Mixin-Typ außerhalb der unterstützten Pfade hinzugefügt, so wird dies ignoriert. |
...
* `DurboImportConfiguration.isImportAcl()` wird wörtlich interpretiert und betrifft nur die Richtlinien zur Zugangskontrolle, die `javax.jcr.security.AccessControlList`
...
Es gibt eine Einschränkung hinsichtlich der Replikation von CUG-Richtlinien. If a given CUG policy gets removed without removing the corresponding mixin node type `rep:CugMixin,` the removal will not be reflected upon replication. Dies wurde korrigiert, indem bei Entfernung einer Richtlinie immer der Mixin-Typ entfernt wird. Die Einschränkung macht sich aber möglicherweise bemerkbar, wenn der Mixin-Typ manuell hinzugefügt wurde.
...
Bei der neuen Implementierung wird die Zugriffssteuerungseinrichtung des Standardautorisierungsmodells nicht von der Erstellung, Änderung oder Entfernung von CUGs beeinflusst. Instead a new type of policy called `PrincipalSetPolicy` is applied as additional access control content to the target node. Diese zusätzliche Richtlinie ist ein untergeordnetes Element des Zielknotens und ist ein gleichrangiges Element des Standardrichtlinienknotens, falls dieser vorhanden ist.

**Bearbeiten von CUG-Richtlinien in der Zugriffssteuerungsverwaltung**

Dieses Abweichen von JCR-Resteigenschaften hin zu einer dedizierten Zugriffssteuerungsrichtlinie hat Auswirkungen auf die Berechtigung, die zum Erstellen oder Ändern des Autorisierungsteils des CUG-Features benötigt wird. Since this is considered a modification to access control content, it requires `jcr:readAccessControl` and `jcr:modifyAccessControl` privileges in order to be written to the repository. Daher können nur Inhaltsautoren, die zur Änderung der Zugriffssteuerungsinhalte einer Seite berechtigt sind, diese Inhalte einrichten oder ändern. Im Vergleich dazu war in der alten Implementierung die Berechtigung zum Schreiben regulärer JCR-Eigenschaften ausreichend. Dies führt zu einer Berechtigungseskalation.
...
>[!NOTE]
>
>If you run into issues, it is possible to set up a specific logger at **DEBUG** level on `com.day.cq.auth.impl.cug` to get the output of the migration tool. Weitere Informationen dazu finden Sie unter [Protokollierung](/help/sites-deploying/configure-logging.md).
