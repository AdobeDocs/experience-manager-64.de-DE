...
Experience Manager haben die Wahl zwischen zwei Lösungen für die Verwendung mit Dynamic Media:

* Verwenden Sie Ihre integrierte Instanz von Experience Manager mit *Dynamic Media Classic*.
* Verwenden von *Dynamic Media*, das in Experience Manager integriert ist.

Verwenden Sie die folgenden Kriterien, um zu ermitteln, welche Lösung am besten für Sie geeignet ist:
...
* **[!UICONTROL Interaktive Medien]** : Die  **[!UICONTROL interaktive]** Komponente ist für Karussellbanner, interaktive Bilder und interaktive Videos gedacht, die in ihnen interaktiv sind, wie Hotspots oder Imagemaps. Diese Komponente ist intelligent. Je nachdem, ob Sie ein Bild oder ein Video hinzufügen, stehen Ihnen verschiedene Optionen zur Verfügung. Darüber hinaus reagiert der Viewer. Das heißt, die Bildschirmgröße ändert sich automatisch je nach Bildschirmgröße. Bei allen Viewern handelt es sich um HTML5-Viewer.
...
Bei der Integration von Dynamic Media Classic mit Experience Manager sollten einige wichtige Best Practices in den folgenden Bereichen beachtet werden:
...
1. Wenn Sie möchten, dass Experience Manager-Assets automatisch in Dynamic Media Classic hochgeladen werden, aktivieren Sie die Option und fügen Sie Dynamic Media Classic zum DAM-Arbeitsablauf für die Aktualisierung von Assets hinzu. Außerdem können Sie manuell Assets hochladen.
1. Hinzufügen von Dynamic Media Classic-Komponenten zum Sidekick. Auf diese Weise können Benutzer Dynamic Media Classic-Komponenten auf ihren Experience Manager-Seiten verwenden.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>Ad-hoc-Ordner</td> 
   <td>Assets, die sich außerhalb des Ordners "Zielgruppe"befinden und manuell in Dynamic Media Classic im WCM hochgeladen werden, werden in Dynamic Media Classic automatisch in einen separaten Ad-hoc-Ordner abgelegt. Sie konfigurieren diese Funktion in der Cloud-Konfiguration in Experience Manager.</td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td><p>Wenn Sie Unterordner im Ordner "Zielgruppe"verwenden, verwenden Sie entweder eindeutige Namen für jedes Asset (unabhängig vom Speicherort) oder konfigurieren Sie Dynamic Media Classic (im Bereich "Einstellungen"), um Assets unabhängig vom Speicherort nicht zu überschreiben.</p> <p>Andernfalls werden Assets mit demselben Namen, die in einen Unterordner der Dynamic Media Classic-Zielgruppe hochgeladen werden, hochgeladen, der gleichnamige Asset im Ordner "Zielgruppe"wird jedoch gelöscht. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
>[!NOTE]
>
>Sie können über mehrere Konfigurationen verfügen: Eine Cloud-Konfiguration stellt einen Benutzer in einer Dynamic Media Classic-Firma dar. Wenn Sie auf andere Dynamic Media Classic-Firmen oder -Benutzer zugreifen möchten, erstellen Sie mehrere Konfigurationen.
...
1. Ändern Sie die Zahl wie gewünscht im Feld **[!UICONTROL Zeitüberschreitung bei aktiven Aufträgen]**. Jede positive Zahl wird binnen Sekunden mit der Maßeinheit akzeptiert. Standardmäßig ist diese Zahl auf 2100 eingestellt.
   ...
   >[!NOTE]
   >
   >Best Practice: Bei den meisten Assets dauert das Übernehmen höchstens einige Minuten (beispielsweise bei Bildern). In bestimmten Fällen, wie z. B. bei größeren Videos, erhöhen Sie den Timeout-Wert auf 7200 Sekunden (zwei Stunden), um die lange Verarbeitungszeit zu reduzieren. Andernfalls wird dieser Dynamic Media Classic-Upload-Auftrag in den JCR-Metadaten als **[!UICONTROL UploadFehlgeschlagen]** markiert.

1. Tippen Sie auf **[!UICONTROL Speichern]**.

### Automatisches Hochladen von Experience Manager-Assets {#autouploading-from-aem-assets}

Ab Experience Manager 6.3.2 sind Experience Manager Assets jetzt für Sie konfiguriert, sodass alle digitalen Assets, die Sie in den Digital Asset Manager hochladen, automatisch in Dynamic Media Classic aktualisiert werden, wenn sich die Assets in einem Ordner mit der CQ-Zielgruppe befinden.
...
1. [Verwenden Sie die Admin Console, um einen Support-Fall zu erstellen.](https://helpx.adobe.com/de/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html) In Ihrem Supportfall fordern Sie an, dass die sichere Vorschau für Ihr Dynamic Media Classic-Konto aktiviert ist.
...
1. Geben Sie im Wertefeld **[!UICONTROL jobParam]** einen Dynamic Media Classic-Upload-Auftragsparameter an.
   ...
   >
   >Achten Sie darauf, dass Ihre PSD-Datei über „Ebenen“ verfügt. Wenn es sich um ausschließlich ein Bild oder um ein Bild mit Maske handelt, wird es nur als Bild verarbeitet, da keine Ebenen verarbeitet werden können.
...
* Ist dies nicht der Fall, müssen Sie die Cloud-Konfiguration unter **[!UICONTROL Seiteneigenschaften]** für diese Seite konfigurieren, um das Hochladen in den Ordner **[!UICONTROL CQ Adhoc]** zu ermöglichen.
