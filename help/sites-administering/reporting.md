...
![reportcomponent](assets/reportcomponent.png) ![reportcomponent](assets/reportcompentall.png)
...
Der Seitenaktivitätsbericht bezieht alle Informationen aus dem Auditprotokoll. By default the root path is configured to the audit log at `/var/audit/com.day.cq.wcm.core.page`.
...
>[!CAUTION]
>
>Access to these reports may be a security issue; therefore we recommend you configure the Dispatcher so that `/etc/reports` is not available to external visitors. Weitere Informationen finden Sie unter [Sicherheitscheckliste](security-checklist.md).
