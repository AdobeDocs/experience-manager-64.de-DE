...
* Hintergrundbilder: Diese werden mithilfe von CSS (Cascading Stylesheets) anstelle von HTML erreicht. Dies bedeutet, dass es nicht möglich ist, einen alternativen Textwert anzugeben. Daher sollten Hintergrundbilder keine wichtigen Textinformationen liefern - wenn dies der Fall ist, müssen diese Informationen auch im Text der Seite angegeben werden.
...
* **Listen verwenden**: Mit HTML können Sie drei verschiedene Arten von Listen angeben:
   ...
   * The `<ol>` element is used for *numbered lists*. Einzelne Listenelemente werden mit dem Element `<li>` gekennzeichnet.
