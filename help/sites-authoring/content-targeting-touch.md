...
1. Standardmäßig ist keine Orts-ID festgelegt. Klicken oder tippen Sie auf das Zahnrad (Konfiguration), um den Ort festzulegen.
   ...
   >
   >Administratoren können unter **festlegen, ob diese Einstellung zwingend vorgenommen werden muss.https://&lt;Host>: /system/console/configMgr/com.day.cq.personalization.impl.servlets.TargetingConfigurationServlet**
 ...
 <tbody> 
  <tr> 
   ...
   <td><p>Die Prozentzahl der Besucher, die auf einen beliebigen Teil des getesteten Erlebnisses geklickt haben. Eine Konversion kann entweder einmal pro Besucher oder jedes Mal, wenn ein Besucher eine Umrechnung durchführt, gezählt werden. Die Konversionsmetrik ist auf einen der folgenden Werte eingestellt:</p> 
    <ul> 
     <li><strong>Seitenansicht</strong> - Sie können festlegen, welche Audience angezeigt wird, indem Sie entweder die <strong>URL auswählen</strong> und dann die URL oder mehrere URLs definieren oder indem Sie die <strong>URL enthält</strong> und dann einen Pfad oder einen Suchbegriff hinzufügen.</li> 
